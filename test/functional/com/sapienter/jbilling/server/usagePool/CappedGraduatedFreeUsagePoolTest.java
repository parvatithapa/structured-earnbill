...
/**
 * CappedGraduatedFreeUsagePoolTest
 * Test class for Capped Graduated with FUP scenarios.
 * @author Amol Gadre
 * @since 15-Dec-2013
 */

@Test(groups = { "usagePools" }, testName = "CappedGraduatedFreeUsagePoolTest")
public class CappedGraduatedFreeUsagePoolTest extends AbstractFreeUsagePoolPricingTest {

	public CappedGraduatedFreeUsagePoolTest() {
		logger.debug("CappedGraduatedFreeUsagePoolTest");
	}

	private static final BigDecimal ORDER_LINE_PRICE = new BigDecimal("0.50");
	private static final BigDecimal MAXIMUM_CHARGED = new BigDecimal("150.00");
	...
	protected void orderLinePriceAssert(OrderWS order, String scenario, BigDecimal removeOrderLineQuantity) {
		...
		if (scenario.equals("ORDER_LINE_PRICE_SCENARIO_1")) {
			...
			// if order total charges > MAXIMUM_CHARGED then total charges = MAXIMUM_CHARGED 
	    	assertEquals("Expected Order total: ", MAXIMUM_CHARGED , order.getTotalAsDecimal());
			...
			if (expectedTotal.compareTo(MAXIMUM_CHARGED) > 0)	
				expectedTotal = MAXIMUM_CHARGED;
			...
			assertEquals("Expected Order total: ", new BigDecimal("75"), order.getTotalAsDecimal());
		}
	}

    @Override
    protected PriceModelWS getTestPriceModel() {
        PriceModelWS priceModel = new PriceModelWS(PriceModelStrategy.CAPPED_GRADUATED.name(), ORDER_LINE_PRICE, Constants.PRIMARY_CURRENCY_ID);
        priceModel.addAttribute("included", "100");     // 100 units included
        priceModel.addAttribute("max", MAXIMUM_CHARGED.toString());     // $100 maximum charged
        return priceModel;
    }
}
