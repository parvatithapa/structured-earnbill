...
/**
 * MeteredFreeUsagePoolTest
 * Test class for Flat Pricing with FUP scenarios.
 * @author Amol Gadre
 * @since 15-Dec-2013
 */

@Test(groups = { "usagePools" }, testName = "MeteredFreeUsagePoolTest")
public class MeteredFreeUsagePoolTest extends AbstractFreeUsagePoolPricingTest {

	public MeteredFreeUsagePoolTest() {
		logger.debug("MeteredFreeUsagePoolTest");
	}
	...
	public void test015MultipleUsagePoolsWithMultpileOrderLines() throws Exception {
		...
		PriceModelWS meteredPrice = getTestPriceModel();
		...
		planId = createPlanForMultpileUsagePool(meteredPrice, usagePoolsId, itemTypeId);
	...
	protected void orderLinePriceAssert(OrderWS order, String scenario, BigDecimal removeOrderLineQuantity) {
		...
		if (scenario.equals("ORDER_LINE_PRICE_SCENARIO_1")) {
			...
			// 100 - 100 = 0,  0 * 0.50 = 0 --> Expected Order line amount
			...
			// (300-300+200) - 100 = 100,  100 * 0.50 = 50 --> Expected Order line amount
			// 50/200 = 0.25   ------> Expected Order line Price
			...
			// (300-300+100) - 100 = 0,  0 * 0.50 = 0 --> Expected Order line amount
			// 0/300 = 0   ------> Expected Order line Price
			...
			// 250 - 100 = 150,  150 * 0.50 = 75 --> Expected Order line amount
			// 75/250 = 0.30 ------> Expected Order line Price

			OrderLineWS orderLine = order.getOrderLines()[0];
			assertEquals("Expected Order line Price: ", new BigDecimal("0.30"), orderLine.getPriceAsDecimal());
			...
			// 220 - 100 = 120,  120 * 0.50 = 60 --> Expected Order line amount
			// 60/250 = 0.272727273 ------> Expected Order line Price

			OrderLineWS orderLine = order.getOrderLines()[0];
			assertEquals("Expected Order line Price: ", new BigDecimal("0.272727273"), orderLine.getPriceAsDecimal());
			assertEquals("Expected Order line Amount: ", new BigDecimal("60.00"), orderLine.getAmountAsDecimal());

		} else if (scenario.equals("ORDER_LINE_PRICE_SCENARIO_17")) {
			// order line qty =	220, increased by 50, total=270, FUP Quantity = 100
			// 270 - 100 = 170,  170 * 0.50 = 85 --> Expected Order line amount
			// 85/270 = 0.314814815 ------> Expected Order line Price

			OrderLineWS orderLine = order.getOrderLines()[0];
			assertEquals("Expected Order line Price: ", new BigDecimal("0.314814815"), orderLine.getPriceAsDecimal());
			assertEquals("Expected Order line Amount: ", new BigDecimal("85.00"), orderLine.getAmountAsDecimal());
