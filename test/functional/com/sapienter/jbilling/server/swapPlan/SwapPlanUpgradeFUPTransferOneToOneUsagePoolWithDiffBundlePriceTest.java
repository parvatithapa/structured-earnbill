 ...
 * 
 */
@Test(groups = { "swapPlan" }, testName = "SwapPlanUpgradeFUPTransferOneToOneUsagePoolWithDiffBundlePriceTest")
public class SwapPlanUpgradeFUPTransferOneToOneUsagePoolWithDiffBundlePriceTest {

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanUpgradeFUPTransferOneToOneUsagePoolWithDiffBundlePriceTest.class);
    ...
    /**
      # Plan Swap - Upgrade of plan F1 Scenario
     ...
     */
    @Test
	public void test001SwapPlanFUPTransferScenarioF1() throws Exception {
    	//Customer F1 with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer F1 - ");
		userId = user.getId();
    ...
    /**
      # Plan Swap - Upgrade of plan F2 Scenario
	  # Old plan containing one usage pool to new plan also containing one usage pool
      # A plan subscription is upgraded from 100 mins free to 225 mins free
      # Bundle Item Price for usage product is different on both plans
     ...
     */
    @Test
	public void test002SwapPlanFUPTransferScenarioF2() throws Exception {
    	//Customer F2 with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer F2 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
        ...
        assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("10.23"), 
		...
		OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(retOrder, 
				afIntroPlanwith100Min.getItemId(), 
				afbestvaluePlanWith225Min.getItemId(), 
				SwapMethod.DIFF,  Util.truncateDate(retOrder.getActiveSince()));
    ...
    /**
     # Plan Swap - Upgrade of plan F3 Scenario
	 # Old plan containing one usage pool to new plan also containing one usage pool
     # A plan subscription is upgraded from 100 mins free to 225 mins free.
     # Bundle Item Price for usage product is different on both plans
     # Overage before the plan swap but no overage after the plan swap.
     */
    @Test
	public void test003SwapPlanFUPTransferScenarioF3() throws Exception {
    	//Customer F3 with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer F3 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
