...
@Test(groups = { "swapPlan" }, testName = "SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTest")
public class SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTest{

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTest.class);
	...
	/**
	 ...
	 # A plan subscription is Upgrade from (pool 225+ pool 85) mins  with bundle price $0.95 free to (Pool 100) mins  with bundle price $1.39 free
	 # No Overage before and after the plan swap.
	 */
	@Test
	public void test001SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTestI4 () throws Exception{
		//Customer 'Customer I4' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer I4 - ");
		userId = user.getId();
		...
		OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(order, downGradePlanwith225Min.getItemId()
				,downGradePlanwith100Min.getItemId(), SwapMethod.DIFF, Util.truncateDate(order.getActiveSince()));
	...
	/**
	 # Plan Swap - Upgrade of a plan
	 # Old plan containing multiple usage pools to new plan containing single usage pools
	 # A plan subscription is Upgrade from (pool 255+ pool 85) mins  with bundle price $0.95 free to (Pool 100) mins free  with bundle price $1.39
	 # Overage before the plan swap and overage after the plan swap.
	 */
	@Test
	public void test002SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTestI5 () throws Exception{
		//Customer 'Customer I5' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer I5 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
		...
		assertEquals("After Swap Plan One Time Order Overage Charges Should be : ",new BigDecimal("167.30"),
				oneTimeUsageOrder.getTotalAsDecimal().setScale(Constants.BIGDECIMAL_SCALE_STR, BigDecimal.ROUND_HALF_UP));
	...
	/**
	 # Plan Swap - Upgrade of a plan
	 # Old plan containing multiple usage pools to new plan containing single usage pools
	 # A plan subscription is Upgrade from (pool 255+ pool 85) mins with bundle price $0.95 free to (Pool 100) mins  with bundle price $1.39 free
	 # No Overage before the plan swap but overage after the plan swap.
	 */
	@Test
	public void test003SwapPlanDowngradeFUPTransferwithMultipleToSinglePoolsWithDiffBundlePriceTestI6 () throws Exception{
		//Customer 'Customer I6' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer I6 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
		...
		assertEquals("After Swap Plan One Time Order Overage Charges Should be : ",new BigDecimal("96.41"),
				oneTimeUsageOrder.getTotalAsDecimal().setScale(Constants.BIGDECIMAL_SCALE_STR, BigDecimal.ROUND_HALF_UP));
