 ...
 *
 */
@Test(groups = { "swapPlan" }, testName = "SwapPlanDowngradeFUPTransferwithOneToMultiplePlansTest")
public class SwapPlanDowngradeFUPTransferwithOneToMultiplePlansTest {

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanDowngradeFUPTransferwithOneToMultiplePlansTest.class);
    ...
    protected void setUp() throws Exception {
    	...
    	planWith135FreeMin = FullCreativeUtil.createPlan("135", "0.95", new Integer[]{usagePoolWith135Quantity.getId()},"Test Plan 135 Min", api, TestConstants.CHAT_USAGE_PRODUCT_ID, TestConstants.INBOUND_USAGE_PRODUCT_ID, TestConstants.ACTIVE_RESPONSE_USAGE_PRODUCT_ID);
    	assertNotNull("planWS should not be null", planWith135FreeMin);

    }	
    
    	@Test
    	public void testScenarioE4() throws Exception {
    		...
    		Integer usageOrderId = FullCreativeUtil.createOneTimeOrder(user.getId(), FullCreativeUtil.getDate(11, 15, 2015),"63.60","36.40","12.20");
    		...
    		assertEquals("Available Quantity of customer usage pool Should be : ",new BigDecimal("66.03"), 
    				customerAvailableUsagePoolQuantity.setScale(Constants.BIGDECIMAL_SCALE_STR, BigDecimal.ROUND_HALF_UP));

    		assertEquals("Mediated Quantity Should be : ",new BigDecimal("112.20"), 
    				FullCreativeUtil.getOrderTotalQuantity(usageOrder));

    		assertEquals("Usage Order Used free quantity Should not null", "112.2000000000", 
    		...
    		OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(subscriptionOrderWith225Min, planWith225FreeMin.getItemId(), planWith135FreeMin.getItemId(), SwapMethod.DIFF, Util.truncateDate(new Date()));
    		assertNotNull("Swap changes should be calculated", orderChanges);
    		Integer subscriptionOrderIdWith135Min = api.createUpdateOrder(subscriptionOrderWith225Min, orderChanges);

    		OrderWS subscriptionOrderWith135Min = api.getOrder(subscriptionOrderIdWith135Min);
    		assertNotNull("### Order after swap plan should not be null", subscriptionOrderWith135Min);

    		Integer swapedPlanId = 0;
    		for (OrderLineWS lineWS:subscriptionOrderWith135Min.getOrderLines()) {
    			if(lineWS.getItemId().equals(planWith135FreeMin.getItemId())) {
    				swapedPlanId = lineWS.getItemId(); 
    			}
    		}

    		assertEquals("##Subscription Order's swaped Plan ID must be", planWith135FreeMin.getItemId(),swapedPlanId);

    		customerUsagePools  = api.getCustomerUsagePoolsByCustomerId(user.getCustomerId());

    		expectedProratedQuatity = new BigDecimal("128.8710");
    		actualProratedQuantity = FullCreativeUtil.getTotalFreeUsageQuantity(customerUsagePools);
    		logger.debug("ActualProratedQuantity:::::::::::::::{}", actualProratedQuantity);
    		assertEquals("Usage pool Prorated quantity should be ",expectedProratedQuatity.setScale(Constants.BIGDECIMAL_QUANTITY_SCALE,BigDecimal.ROUND_HALF_UP), actualProratedQuantity.setScale(Constants.BIGDECIMAL_QUANTITY_SCALE,BigDecimal.ROUND_HALF_UP));

    		assertEquals("Expected Cycle start date of customer usage pool: ", TestConstants.DATE_FORMAT.format(FullCreativeUtil.getDate(11, 15, 2015))
    				,TestConstants.DATE_FORMAT.format(FullCreativeUtil.getCustomerUsagePoolCycleStartDateByPlanId(customerUsagePools, planWith135FreeMin.getId())));

    		assertEquals("Expected Cycle end date of customer usage pool: ", TestConstants.DATE_FORMAT.format(FullCreativeUtil.getDate(11, 31, 2015))
    				,TestConstants.DATE_FORMAT.format(FullCreativeUtil.getCustomerUsagePoolCycleEndDateByPlanId(customerUsagePools, planWith135FreeMin.getId())));
    		...
    		assertEquals("Available Quantity of customer usage pool Should be : ",new BigDecimal("16.67"), 
    				customerAvailableUsagePoolQuantity.setScale(Constants.BIGDECIMAL_SCALE_STR, BigDecimal.ROUND_HALF_UP));
    	...
    	@Test
    	public void testScenarioE5() throws Exception {
    		//Customer 'Swap Plan Proration' with Next Invoice Date 01-01-2016
    		UserWS user  = FullCreativeUtil.createUser("Swap Plan Proration");
    		userId = user.getId();
    		assertNotNull("user should not be null",user);
    		...
    		assertEquals("Usage Order Used free quantity Should not null", "128.8710000000", 
    				FullCreativeUtil.getTotalFreeUsageQuantityByOrder(usageOrder).toString());

    		assertEquals("One Time Order Overage Quantity Should be : ",new BigDecimal("52.33"), 
    				overageQuantity.setScale(2, BigDecimal.ROUND_HALF_UP));

    		assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("49.71"), 
    				usageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
    	}
    
    
    	@Test	
    	public void testScenarioE6() throws Exception {
    		//Customer 'Swap Plan Proration' with Next Invoice Date 01-01-2016
    		UserWS user  = FullCreativeUtil.createUser("Swap Plan Proration");
    		userId = user.getId();
    		assertNotNull("user should not be null",user);
    		...
    		Integer usageOrderId = FullCreativeUtil.createOneTimeOrder(user.getId(), FullCreativeUtil.getDate(11, 15, 2015),"65.60","36.40","32.20");
    		...
    		assertEquals("Available Quantity of customer usage pool Should be : ",new BigDecimal("44.03"), 
    				customerAvailableUsagePoolQuantity.setScale(Constants.BIGDECIMAL_SCALE_STR, BigDecimal.ROUND_HALF_UP));

    		assertEquals("Mediated Quantity Should be : ",new BigDecimal("134.20"), 
    				FullCreativeUtil.getOrderTotalQuantity(usageOrder));

    		assertEquals("Usage Order Used free quantity Should not null", "134.2000000000", 
    				FullCreativeUtil.getTotalFreeUsageQuantityByOrder(usageOrder).toString());
    		...
    		assertEquals("One Time Order Overage Quantity Should be : ",new BigDecimal("5.33"), 
    				overageQuantity.setScale(2, BigDecimal.ROUND_HALF_UP));

    		assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("5.06"), 
    				usageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
    	}
}
