 ...
 *
 */
@Test(groups = { "swapPlan" }, testName = "SwapPlanUpgradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest")
public class SwapPlanUpgradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest {

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanUpgradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest.class);
    ...
    /**
   	# Plan Swap - Upgrade of plan H1 Scenario
       ...
       */
    @Test
    public void testScenarioH1() throws Exception {
    	//Customer 'Customer H1' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H1 -");
    	userId = user.getId();
    	...
    	OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(order, upGradePlanwith100Min.getItemId(), upGradePlanwith225Min.getItemId(), SwapMethod.DIFF, Util.truncateDate(order.getActiveSince()));
    ...
    /**
	# Plan Swap - Upgrade of plan H2 Scenario
	# Old plan containing one usage pool to new plan also containing one usage pool
	# A plan subscription is upgraded from 100 mins free to 225 mins free.
	# Bundle Item Price for usage product is different on both plans. 
   ...
   */
    @Test
    public void testScenarioH2() throws Exception {
    	//Customer 'Customer H2 -' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H2 -");
    	userId = user.getId();
    	assertNotNull("user should not be null",user);
    	user = api.getUserWS(userId);
    	...
    	assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("93.63"), 
    ...
    /**
   	# Plan Swap - Upgrade of plan H3 Scenario
   	# Old plan containing one usage pool to new plan also containing one usage pool
   	# A plan subscription is upgraded from 100 mins free to 225 mins free.
   	# Bundle Item Price for usage product is different on both plans. 
      ...
      */
    @Test
    public void testScenarioH3() throws Exception {
    	//	Customer 'Swap Plan Proration' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H3 -");
    	userId = user.getId();
    	assertNotNull("user should not be null",user);
    	user = api.getUserWS(userId);
    	...
    	assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("167.30"), 
    			usageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
