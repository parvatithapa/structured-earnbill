 ...
 *
 */
@Test(groups = { "swapPlan" }, testName = "SwapPlanDowngradeFUPTransferwithOneToOneUsagePoolTest")
public class SwapPlanDowngradeFUPTransferwithOneToOneUsagePoolTest {

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanDowngradeFUPTransferwithOneToOneUsagePoolTest.class);
    ...
    /**
     # Plan Swap - Upgrade of plan A4 Scenario
     ...
     */
    @Test
	public void test001SwapPlanFUPTransferScenarioA4() throws Exception {
     ...
     */
    @Test
   	public void test002SwapPlanFUPTransferScenarioA5() throws Exception {
		...
		logger.debug("Swapped to Plan Id :::{}", afIntroPlanwith100Min.getId());
   		...
   		assertEquals("After Swap Plan One Time Order Overage Charges Should be : ",new BigDecimal("63.99"), 
    ...
    /**
       # Plan Swap - Upgrade of plan A6 Scenario
 	   # Old plan containing one usage pool to new plan also containing one usage pool
       # A plan subscription is downgraded from 225 mins free to 100 mins free. 
       # Bundle Item Price for usage product remains the same on both plans.
     ...
     */
    @Test
   	public void test003SwapPlanFUPTransferScenarioA6() throws Exception {
       	//Customer A6 with Next Invoice Date 01-01-2016
   		UserWS user  = FullCreativeUtil.createUser("Customer A6 - ");
   		userId = user.getId();
   		assertNotNull("user should not be null",user);
   		user.setNextInvoiceDate(FullCreativeUtil.getDate(0, 1, 2016));
   		api.updateUser(user);
		...
		OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(retOrder, 
           ...
           assertEquals("After Swap Plan One Time Order Overage Charges Should be : ",new BigDecimal("6.99"), 
           		oneTimeUsageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
