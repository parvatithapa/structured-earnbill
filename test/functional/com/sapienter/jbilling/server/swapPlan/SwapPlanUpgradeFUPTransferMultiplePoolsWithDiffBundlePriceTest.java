...
@Test(groups = { "swapPlan" }, testName = "SwapPlanUpgradeFUPTransferMultiplePoolsWithDiffBundlePriceTest")
public class SwapPlanUpgradeFUPTransferMultiplePoolsWithDiffBundlePriceTest{

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanUpgradeFUPTransferMultiplePoolsWithDiffBundlePriceTest.class);
	...
	/**
      # Plan Swap - Upgrade of plan G1 Scenario 
   ...
   */
	@Test
	public void test001SwapPlanUpgradeFUPTransferwithMultiplePoolsTestG1 () throws Exception {
		//Customer 'Customer G1' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer G1 - ");
		userId = user.getId();
		...
		//Create subscription order for 100 min plan (100 + 135 Min Usage pools) with Active since date 12-15-2015
		...
		assertEquals("Available Quantity of customer usage pool Should be : ",new BigDecimal("6.67"), 
        		customerUsagePoolQuantity.setScale(Constants.BIGDECIMAL_SCALE_STR,BigDecimal.ROUND_HALF_UP));
		
		// Swapping 100 min Plan (100 + 135 MIN Usage Pools) with 225 Min Plan (225 + 85 MIN Usage pools)
		OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(order, 
				upGradePlanwith100Min.getItemId(), 
				upGradePlanwith225Min.getItemId(), 
				SwapMethod.DIFF, Util.
				truncateDate(order.getActiveSince()));
	 ...
	 */
	@Test
	public void test002SwapPlanUpgradeFUPTransferwithMultiplePoolsTestG2 () throws Exception {
		//Customer 'Customer G2' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer G2 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
		...
		assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("18.53"), 
	...
	/**
      # Plan Swap - Upgrade of plan G3 Scenario 
      # Old plan containing multiple usage pools to new plan also containing multiple usage pools
	  # A plan subscription is Upgrade from (pool 100+ pool 135) mins free to (Pool 225+ Pool 85) mins free
	  # Bundle Item Price for usage product is different on both plans
	 ...
	 */
	@Test
	public void test003SwapPlanUpgradeFUPTransferwithMultiplePoolsTestG3 () throws Exception{
		//Customer 'Customer G3' with Next Invoice Date 01-01-2016
		UserWS user  = FullCreativeUtil.createUser("Customer G3 - ");
		userId = user.getId();
		assertNotNull("user should not be null",user);
		user = api.getUserWS(userId);
		...
		assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("64.40"), 
				oneTimeUsageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
