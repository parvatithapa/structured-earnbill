 ...
 *
 */
@Test(groups = { "swapPlan" }, testName = "SwapPlanDowngradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest")
public class SwapPlanDowngradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest {

	private static final Logger logger = LoggerFactory.getLogger(SwapPlanDowngradeFUPTransferwithOneToMultipleUsagePoolsWithDiffBundlePriceTest.class);
    ...
    /**
	# Plan Swap - DownGrade of plan H4 Scenario
   ...
   */
    @Test
    public void testScenarioH4() throws Exception {
    	//Customer 'Customer H4' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H4 -");
    	userId = user.getId();
    	...
    	OrderChangeWS[] orderChanges = api.calculateSwapPlanChanges(order, downGradePlanwith225Min.getItemId(), downGradePlanwith100Min.getItemId(), SwapMethod.DIFF, Util.truncateDate(order.getActiveSince()));
    ...
    /**
	# Plan Swap - DownGrade of plan H5 Scenario
	# Old plan containing one usage pool to new plan also containing one usage pool
	# A plan subscription is downgraded from 225 mins free to 100 mins free. 
	# Bundle Item Price for usage product is different on both plans.  
     ...
     */
    @Test
    public void testScenarioH5() throws Exception {
    	//Customer 'Customer H5' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H5 -");
    	userId = user.getId();
    	assertNotNull("user should not be null",user);
    	user = api.getUserWS(userId);
    	...
    	assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("71.93"), 
    			usageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
    } 

    /**
	# Plan Swap - Downgrade of plan H6 Scenario
	# Old plan containing one usage pool to new plan also containing one usage pool
	# A plan subscription is downgraded from 225 mins free to 100 mins free. 
	# Bundle Item Price for usage product is different on both plans.  
     ...
     */
    @Test
    public void testScenarioH6() throws Exception {
    	//Customer 'Swap Plan Proration' with Next Invoice Date 01-01-2016
    	UserWS user  = FullCreativeUtil.createUser("Customer H6 -");
    	userId = user.getId();
    	assertNotNull("user should not be null",user);
    	user = api.getUserWS(userId);
    	...
    	assertEquals("One Time Order Overage Charges Should be : ",new BigDecimal("24.67"), 
    			usageOrder.getTotalAsDecimal().setScale(2, BigDecimal.ROUND_HALF_UP));
    } 
}
