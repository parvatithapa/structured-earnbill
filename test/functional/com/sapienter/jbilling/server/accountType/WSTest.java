...
@Test(groups = {"web-services", "accountType"}, testName = "accountType.WSTest")
public class WSTest extends ApiTestCase {
	...
	public void test004UpdateAccountType() {
		...
		refAccountType.setName("Updated Account Type", Constants.LANGUAGE_ENGLISH_ID);
	...
	@Test(expectedExceptions = SessionInternalError.class,
			expectedExceptionsMessageRegExp = "(?s).*AccountInformationTypeWS,accountTypeId,validation.error.notnull.*")
	...
	@Test
	public void test030TestAccountTypeWithMultipleAit() {
		//create local account type
		AccountTypeWS accountType = new AccountTypeBuilder().create(api);
		assertNotNull("Account Type Should be created", accountType.getId());
		...
		AccountInformationTypeWS aitOne = buildValidAitWithMetaFields(accountType);
		aitOne.setName("First Contact");
		aitOne.setUseForNotifications(true);
		aitOne.setId(api.createAccountInformationType(aitOne));
		assertNotNull("Can't save account information type", aitOne.getId());
		...
		AccountInformationTypeWS aitTwo = buildValidAitWithMetaFields(accountType);
		aitTwo.setName("Second Contact");
		aitTwo.setUseForNotifications(true);
		aitTwo.setId(api.createAccountInformationType(aitTwo));
		assertNotNull("Can't save account information type", aitTwo.getId());

		//create the third Ait
		AccountInformationTypeWS aitThree = buildValidAitWithMetaFields(accountType);
		aitThree.setName("third Contact");
		aitThree.setUseForNotifications(true);
		aitThree.setId(api.createAccountInformationType(aitThree));
		assertNotNull("Can't save account information type", aitThree.getId());

		//create the forth Ait
		AccountInformationTypeWS aitfour = buildValidAitWithMetaFields(accountType);
		aitfour.setName("fourth Contact");
		aitfour.setUseForNotifications(true);
		aitfour.setId(api.createAccountInformationType(aitfour));
		assertNotNull("Can't save account information type", aitfour.getId());

		//create the fifth Ait
		AccountInformationTypeWS aitfive = buildValidAitWithMetaFields(accountType);
		aitfive.setName("Fifth Contact");
		aitfive.setId(api.createAccountInformationType(aitfive));
		assertNotNull("Can't save account information type", aitfive.getId());

		api.updateAccountType(accountType);
		accountType = api.getAccountType(accountType.getId());

		assertEquals("Count Of Prefered AITs is ", 4, accountType.getPreferedInformationTypeIds().length);
	...
	private void matchAccountType(AccountTypeWS accountType, AccountTypeWS accountTypeRetrieved) {
		assertEquals(accountType.getDescription(Constants.LANGUAGE_ENGLISH_ID).getContent(),
				accountTypeRetrieved.getDescription(Constants.LANGUAGE_ENGLISH_ID).getContent());
