 ...
 * @since Oct-21-2011
 */
@Test(groups = { "web-services", "process" }, testName = "process.WSTest")
public class WSTest {
    ...
    public void testMultiPeriodGenerateInvoice() throws Exception {
        ...
        order.setPeriod(2); // monthly
        order.setCurrencyId(1);
        order.setActiveSince(new DateMidnight(2011, 7, 1).toDate()); // active since August 01, 2011
        order.setActiveUntil(new DateMidnight(2011, 8, 1).toDate()); // active until September 01, 2011
        ...
        secondOrder.setBillingTypeId(Constants.ORDER_BILLING_PRE_PAID);
        secondOrder.setPeriod(2); // monthly
        secondOrder.setCurrencyId(1);
        secondOrder.setActiveSince(new DateMidnight(2011, 8, 1).toDate()); // active since September 01, 2011

        // create an order to be invoiced
        OrderLineWS secondLine = new OrderLineWS();
        secondLine.setTypeId(Constants.ORDER_LINE_TYPE_ITEM);
        ...
        for (OrderChangeWS change: secondOrderChanges) {
            change.setStartDate(new DateMidnight(2011, 9, 1).toDate());
        }
        ...
        //new line should not be applied
        assertEquals("Invoice line shouldn't be filled with the Lemonade product and delegated invoice", 1, secondInvoice.getInvoiceLines().length);
    ...
    public void testGenerateInvoiceMultiplePartialPeriod() throws Exception {
        ...
        for (InvoiceLineDTO invoiceLine : invoice.getInvoiceLines()) {
            ...
            if ("Lemonade  Period from 02/20/2011 to 02/28/2011".equals(invoiceLine.getDescription()))
                period20to28 = true;
            if ("Lemonade  Period from 02/10/2011 to 02/20/2011".equals(invoiceLine.getDescription()))
                period10to20 = true;
        }

        assertTrue("Pro-rated between Feb 10 -> 28", period10to28);
        assertTrue("Pro-rated between Feb 20 -> 28", period20to28);
        assertTrue("Pro-rated between Feb 10 -> 20", period10to20);
