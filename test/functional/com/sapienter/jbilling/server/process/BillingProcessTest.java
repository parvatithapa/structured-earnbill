...
public class BillingProcessTest extends BillingProcessTestCase {
    ...
    public void test001EndOfMonthCorrection () throws Exception {
         ...
         * config.setDueDateUnitId(Constants.PERIOD_UNIT_MONTH); config.setDueDateValue(new Integer(1));
         * config.setInvoiceDateProcess(new Integer(1)); config.setMaximumPeriods(new Integer(10));
         * config.setOnlyRecurring(new Integer(1)); config.setPeriodUnitId(Constants.PERIOD_UNIT_MONTH);
        ...
        logger.debug("Invoice ids: {}", Arrays.toString(invoiceIds));
    ...
    public void test002ViewLimit () throws Exception {
        ...
        logger.debug("A - Setting config to: {}", config);
        ...
        logger.debug("::::: testViewLimit user: {}", user);
        ...
        logger.debug("WithDayOfMonth 1 " + new DateMidnight(new Date()).withDayOfMonth(1).toDate());
        logger.debug("Minus 10 Days " + new DateMidnight(new Date()).withDayOfMonth(1).minus(10).toDate());
        logger.debug("Active Since " + order.getActiveSince());
        ...
        logger.debug("View Limit {}",  viewLimit);
        Calendar activeSince= Calendar.getInstance();
        activeSince.setTime(order.getActiveSince());
        logger.debug("Starting with activeSince {}",  activeSince.getTime());
        int i = 0;
        while (activeSince.getTime().compareTo(viewLimit) < 0 ) {
        	activeSince.add(Calendar.MONTH, 1);
        	logger.debug("I at {} and activeSince {}", i,  activeSince.getTime());
        	i++;
        }

        BigDecimal expectedValue = new BigDecimal(60).multiply(new BigDecimal(i));
        logger.debug("Expected value {} actual Value {}", expectedValue, invoice.getBalanceAsDecimal());
    ...
    public void test005Review () throws Exception {
        ...
        ItemDTOEx item = api.getItem(1, null, null);
        item.setStandardPartnerPercentage("10");
        api.updateItem(item);
        ...
        OrderLineWS[] lineNew = {order103.getOrderLines()[0]};

		order103.setOrderLines(lineNew);
        ...
        logger.debug("TODO: should there be already a review?");
        ...
        logger.debug("C - Setting config to: {}", config);
        api.createUpdateBillingProcessConfiguration(config);
        ...
        logger.debug("billing process have run run for date: {}", runDate);
        ...
        for(CommissionProcessRunWS commissionRun : commissionRuns){
            if(new DateTime(commissionRun.getPeriodStart()).toDateMidnight().equals(new DateTime(AsDate(2006, 10, 15)).toDateMidnight()) ){
        ...
        assertTrue("No commissions must be found", commissions == null || commissions.length == 0);

        // get the review
        // now review should be there
        logger.debug("Getting last review");
        ...
        logger.debug("Validating invoice delegation");
        ...
        logger.debug("Review invoice id: {}", reviewInvoiceId);
        ...
        assertEquals("Overdue invoice should remain 'unpaid', since this is only a review",
                Constants.INVOICE_STATUS_UNPAID, invoice.getStatusId());
        ...
        assertEquals("Overdue invoice should remain 'unpaid', after disapproval", Constants.INVOICE_STATUS_UNPAID,
                invoice.getStatusId());

        assertEquals("Overdue invoice balance 15", new BigDecimal("15.0"), invoice.getBalanceAsDecimal());

        logger.debug("Triggering Billing early");
        ...
        boolean status = api.triggerBilling(AsDate(2006, 10, 22));
        assertTrue("Billing for 2006-10-22 expected to be run, but is wasn't", status);
        ...
        try {
            ...
            logger.debug("Invoice: {} for invoiceId: {}",invoice, reviewInvoiceId);
            fail("Review invoice from billing executed at 2006-10-26 still exists, but it shouldn't. Should throw a Hibernate exception instead.");
        } catch (Exception e) {
        }
        ...
        config = api.getBillingProcessConfiguration();
        assertEquals("12 - Review generated", config.getReviewStatus(), Constants.REVIEW_STATUS_GENERATED.intValue());
    ...
    public void test007Process () throws Exception {
        ...
        logger.debug("found running status for process: {}", runningStatus);
        ...
        ProcessStatusWS completedStatus = api.getBillingProcessStatus();
        logger.debug("completed status for process: {}", completedStatus);
        ...
        assertEquals("Overdue invoice is now 'carried over'", Constants.INVOICE_STATUS_UNPAID_AND_CARRIED,
                invoice.getStatusId());
    ...
    public void test010NextBillingProcess () throws Exception {
        ...
        logger.debug("Running the billing process again");
        BillingProcessConfigurationWS config = api.getBillingProcessConfiguration();
        config.setProratingType(ProratingType.PRORATING_AUTO_ON.toString());
        api.createUpdateBillingProcessConfiguration(config);
        config = api.getBillingProcessConfiguration();
    ...
    public void test017NegativeInvoiceToCredit () throws Exception {
        /*
        We will create 2 orders
        - order 1 for $300 and then create an invoice for it.
        - order 2 for $-100 and then create an invoice.
        After the second invoice there should be a credit note for $100 which should be applied to the first invoice.
        The first invoice should then have a balance of $200
         */
        // user for tests
        UserWS user = com.sapienter.jbilling.server.user.WSTest.createUser(true, null, null);
        ...
        line.setItemId(1);
        line.setQuantity(1);
        line.setPrice(new BigDecimal("300.00"));
        ...
        logger.debug("Order ID: {}", orderId1);
        assertNotNull(orderId1);
        Integer[] invoiceIds = api.createInvoice(user.getUserId(), false);
        assertEquals(1, invoiceIds.length);
        Integer invoiceId1 = invoiceIds[0];

        InvoiceWS invoiceWs1 = api.getInvoiceWS(invoiceId1);
        Asserts.assertEquals("Invoice total should be $300.00", new BigDecimal("300.00"), invoiceWs1.getBalanceAsDecimal());
        ...
        //Credit Note should be created. Invoice balance should be 0
        assertEquals("Invoice total should be $0.00", BigDecimal.ZERO, invoice.getBalanceAsDecimal());
        ...
        //credit note should be applied to first invoice
        invoiceWs1 = api.getInvoiceWS(invoiceId1);
        Asserts.assertEquals("Invoice total should be $200.00", new BigDecimal("200.00"), invoiceWs1.getBalanceAsDecimal());

        // Validate that a credit payment should have been created.
        Integer[] creditNoteIds = api.getLastCreditNotes(userId, 1);
        logger.debug("creditNotes:{} ", creditNoteIds.length);
        assertEquals(1, creditNoteIds.length);
        CreditNoteWS creditNote = api.getCreditNote(creditNoteIds[0]);
        assertEquals("The credit payment amount should be $100.00", new BigDecimal("100.00"),
                creditNote.getAmountAsDecimal());
        assertEquals("The credit payment amount should be $0.00", BigDecimal.ZERO,
                creditNote.getBalanceAsDecimal());
    ...
    public void test018BillingProcessFailure () throws Exception {
        ...
        logger.debug("::::: test017BillingProcessFailure user: {}", user);
        ...
        logger.debug("D - Setting config to: {}", config);
        api.createUpdateBillingProcessConfiguration(config);
        logger.debug("Running proces for : {}", runDate);
        api.triggerBilling(runDate);
        ...
        logger.debug("E - Setting config to: {}", config);
        api.createUpdateBillingProcessConfiguration(config);
        logger.debug("Running proces for : {}", runDate);
        api.triggerBilling(runDate);
        ...
        logger.debug("Got out of the job");
    ...
    public void test018NegativeInvoiceToCredit () throws Exception {
        ...
        assertEquals("Payment method should be credit.", Constants.PAYMENT_METHOD_CREDIT, paymentWS.getMethodId());
    ...
    public void test020AssetRemovalWhenFinished () throws Exception {
        ...
        // There should be only 1 transition as the current transition gets updated by setting its end date
        // and changing the new status back to default. No new transition is created.
        assertEquals(
    ...
    public void testIsBillingRunning () {
        ...
        logger.debug("Is billing process running for entity {} : {}", entityId, running);
        ...
        logger.debug("Is billing process running for entity {} : {}",testEntityId, running);
