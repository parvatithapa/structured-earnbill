 ...
 */
@Test(groups = {"web-services", "util"}, testName = "util.WSTest")
public class WSTest {
    ...
    public void test006DeleteEnumerationUsedInMetaField(){
        ...
        rule.addErrorMessage(Constants.LANGUAGE_ENGLISH_ID, "Please enter a value between 1 and 10");
    ...
    @Test
    public void test010FindUsageStatistics() throws Exception {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm");
        JobExecutionHeaderWS[] headers = api.getJobExecutionsForDateRange("testType", sdf.parse("2004-03-09T00:00:00.0"), null, 0, 5, "id", "asc");
        assertEquals("Expecting 3 headers", 3, headers.length);

        boolean foundHeader = false;
        for(JobExecutionHeaderWS header : headers) {
            if(header.getId() == -10) {
                foundHeader = true;
                assertEquals("Job Execution ID must be -1", -1, header.getJobExecutionId());
                assertEquals("Status must be SUCCESS", "SUCCESS", header.getStatus());
                assertEquals("Expecting 2 lines", 2, header.getLines().length);

            }
        }
        assertTrue("Header -10 not found", foundHeader);

        headers = api.getJobExecutionsForDateRange("testType", sdf.parse("2004-03-09T00:00:00.0"), null, 0, 1, "id", "asc");
        assertEquals("Expecting 1 headers",1, headers.length);

        headers = api.getJobExecutionsForDateRange("testType", sdf.parse("2004-03-11T00:00:00.0"), null, 0, 5, "id", "asc");
        assertEquals("Expecting 1 headers",1, headers.length);
    }
     ...
     */
    private static EnumerationWS createEnumeration(String name){
