...
@Test(groups = {"rating-late-usage", "late-usage"}, testName = "LateGuidedUsageTest")
public class LateGuidedUsageTest {
    ...
    public void checkSubscriptionForFutureOrder() {
        ...
        try {
            ...
            logger.debug("TEST OUT: parent order is {}", parentOrder.getId());
            logger.debug("TEST OUT: child order is {}", bundledOrder.getId());
...
// Second Order
    ...
    public void testFutureSubscriptionNoCustomerPrice() throws Exception {
        ...
        logger.debug("TEST OUT: Order set for active since {}", order.getActiveSince());

        order.setId(api.createOrder(order, buildForOrderWithOrderChangeWithStartDate(order, order.getActiveSince()))); // create order
        order = api.getOrder(order.getId());
        assertNotNull("order created", order.getId());
        logger.debug("TEST OUT: order active since = {}", order.getActiveSince());
        ...
        //Test Removed, we can create an order in the future, but the order change will be applied in the future. We
//        testOrder.setActiveSince(order.getActiveSince());
//        testOrder = api.rateOrder(testOrder, buildForOrderWithOrderChangeWithStartDate(testOrder, order.getActiveSince()));
//        assertEquals("Order line should be priced at $5.00.", new BigDecimal("5.00"), testOrder.getOrderLines()[0].getPriceAsDecimal());
    ...
    public void testUpdatedSubscriptionInFutureNoPrice() throws Exception {
        ...
        //Test is not possible as it is done now, we are trying to set an active since after the OrderChange start date
...
//        assertEquals("Order line should be priced at $10.00.", new BigDecimal("10.00"), testOrder.getOrderLines()[0].getPriceAsDecimal());
    ...
    public void testCustomerPricingTestBasedOnSubscriptionDates() throws Exception {
        ...
        logger.debug("order.getActiveUntil() = {}", order.getActiveUntil());
        OrderChangeWS[] orderChangeWSes = OrderChangeBL.buildFromOrder(order, ORDER_CHANGE_STATUS_APPLY_ID);
        orderChangeWSes = Arrays.asList(orderChangeWSes).stream().map(oc -> {
            oc.setStartDate(DateUtils.addDays(new Date(), -61));
            return oc;
        }).collect(Collectors.toList()).toArray(new OrderChangeWS[0]);
        order.setId(api.createOrder(order, orderChangeWSes)); // create order
        order = api.getOrder(order.getId());
        assertNotNull("order created", order.getId());
        ...
        logger.debug("TEST OUT: Price returned for activeSince is {}", api.getCustomerPriceForDate(userId, PRICING_ITEM_ID, order.getActiveSince(), null).getModels());
    ...
    public void testCustomerPricePlanNonPlanPricing() throws Exception {
        ...
        logger.debug("TEST OUT: itemPrice = {}", itemPrice);
     ...
     * TODO MEDIATION 4.0: DUMMY MEDIATION IS NOT ACTIVE
	 */
	@Test
	public void checkCustomerPriceInMediation12609() throws Exception {
        Integer mediationConfigurationId = null;
        try {
            api = JbillingAPIFactory.getAPI();
            ...
            config.setName("WS - checkCustomerPriceInMediation12609");
            config.setMediationJobLauncher("sampleMediationJob");
            config.setGlobal(Boolean.FALSE);
            config.setOrderValue("2002");
            config.setPluggableTaskId(421);
            mediationConfigurationId = api.createMediationConfiguration(config);

            String exampleANumber = "38922734081";
            String exampleBNumber = "38925634973";

            String callDataRecord = StringUtils.join(Arrays.asList("LGUWS-" + random + "_1",
                    exampleANumber, exampleBNumber,
                    "2014-01-01 11:40:11", 100, item.getId(), user.getUserName()
            ), ",");

            UUID processId = api.processCDR(mediationConfigurationId, Arrays.asList(callDataRecord));
            ...
            processId = api.processCDR(mediationConfigurationId, Arrays.asList(
                    StringUtils.join(Arrays.asList("LGUWS-" + random + "_2",
                    exampleANumber, exampleBNumber,
                    "2014-02-15 11:40:11", 100, item.getId(), user.getUserName()
            ), ",")));
            assertNotNull("Process not created", processId);
            ...
            processId = api.processCDR(mediationConfigurationId, Arrays.asList(
                    StringUtils.join(Arrays.asList("LGUWS-" + random + "_3",
                            exampleANumber, exampleBNumber,
                            "2014-01-28 11:40:11", 100, item.getId(), user.getUserName()
                    ), ",")));
            assertNotNull("Process not created", processId);
            ...
            if (mediationConfigurationId != null) api.deleteMediationConfiguration(mediationConfigurationId);
        }
    ...
    private OrderChangeWS[] buildForOrderWithOrderChangeWithStartDate(OrderWS order, Date orderChangeStartDate) {
        return Arrays.asList(OrderChangeBL.buildFromOrder(order, ORDER_CHANGE_STATUS_APPLY_ID)).stream().map(oc -> {
            oc.setStartDate(orderChangeStartDate);
            return oc;
        }).collect(Collectors.toList()).toArray(new OrderChangeWS[0]);
    }
}
