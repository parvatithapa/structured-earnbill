 ...
 *
 */
@Test(groups = { "fupProrating" }, testName = "FUPProratingMultiplePlansActiveSinceChangeTest")
public class FUPProratingMultiplePlansActiveSinceChangeTest {

	private static final Logger logger = LoggerFactory.getLogger(FUPProratingMultiplePlansActiveSinceChangeTest.class);
	...
	/**
	 * FUP Proration B4 Scenario
	 */
	@Test
	public void test004FupProratingActiveUntilDateChangeScenarioB4() {

		try {
			logger.debug("### FUP Proration C1 Scenario - Active Since date change of One of the Monthly order - TWO subscription orders for customer - each containing separate plan - each plan has single usage pool - Overage before but no overage after the date change ###");

			//Customer C1 with Next Invoice Date 01-01-2016
