...
public class PriceModelWSTest extends TestCase {
    ...
    public void testFromDTO() {
        ...
        dto.setId(1);
        dto.setType(PriceModelStrategy.FLAT);
        dto.setAttributes(attributes);
        dto.setRate(new BigDecimal("0.7"));
        dto.setCurrency(new CurrencyDTO(1));

        // convert to PriceModelWS
        PriceModelWS ws = new PriceModelBL().getWS(dto);

        assertEquals(dto.getId(), ws.getId());
        assertEquals("FLAT", ws.getType());
        assertTrue(dto.getRate().compareTo(ws.getRateAsDecimal()) == 0);
