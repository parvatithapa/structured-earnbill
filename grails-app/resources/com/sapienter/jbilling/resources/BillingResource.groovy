...
class BillingResource {
    ...
    @GET
    @Path("/processes/last/invoices/{maxResult}/{offset}")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = "Get the invoices generated by the last billing process run.", response = InvoiceWS.class)
    @ApiResponses(value = [
            @ApiResponse(code = 200, message = "Returning the generated invoices by the last billing process run.",
                    response = InvoiceWS.class),
            @ApiResponse(code = 404, message = "A billing process run was not found."),
            @ApiResponse(code = 500, message = "Fetching the generated invoices failed.")
    ])
    Response getLastBillingProcess(
            @ApiParam(name = "maxResult", value = "The max number of generated invoices.", required = false)
            @PathParam("maxResult")
            Integer maxResult,
            @ApiParam(name = "offset", value = "The next page number from paginated set of value.", required = false)
            @PathParam("offset")
            Integer offset
    ) {
        ...
        try {
            ...
            for (Integer invoiceId : webServicesSession.getPagedBillingProcessGeneratedInvoices(billingRunId, maxResult, offset)) {
