...
class SecurityFilters {
	...
	def filters = {
		...
		twoFactorCheck(controller: '*', controllerExclude: 'jaxrs|login|logout|twoFactorVerify', action: '*') {
			before = {
				Integer companyId = session.getAttribute("company_id")
				boolean is2FAConfiguredInCurrentSession = session.getAttribute("is2FAConfiguredInCurrentSession") as boolean
				if(is2FAConfiguredInCurrentSession || !twoFactorAuthenticationHelperService.is2FAEnable(companyId)) {
					return true
				}
				boolean is2faDone = session.getAttribute("2fa_done") as boolean
				if(springSecurityService.isLoggedIn() && !is2faDone) {
					redirect(controller: "twoFactorVerify", action: "index")
					return false
				}
				return true
			}
