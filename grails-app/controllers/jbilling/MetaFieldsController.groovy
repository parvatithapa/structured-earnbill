...
class MetaFieldsController {
    ...
    def list () {
        ...
        if (params.id && params.get('id').isInteger()) {
            ...
            securityValidator.validateCompany(selectedField?.entityId, Validator.Type.VIEW)
        } else {
            if (params.id) {
        ...
        if (params.id && params.get('id').isInteger() && selectedField) {
        ...
        if (usingJQGrid) {
            if (params.template) {
                render template: 'listMetaFieldsTemplate',
                          model: [selectedCategory: entityType?.name(),
                                          selected: new MetaField()]
            } else {
        ...
        if (params.template) {
            ...
            render(view: 'listCategories', model: [selectedCategory: entityType?.name(), lst: getCategoryList(), lstByCategory: lstByCateg, selected: selectedField])
        }
    }
    ...
    def show () {
        ...
        securityValidator.validateCompany(metaField?.entityId, Validator.Type.VIEW)
    ...
    def edit () {
        ...
        securityValidator.validateCompany(metaField?.entityId, Validator.Type.EDIT)

        if(!metaField.id && params['entityType'] == null){
            redirect action: 'listCategories', params: params
        }
        def descriptions=[]
    ...
    def save () {
        ...
        securityValidator.validateCompany(existingMetaField?.entityId, Validator.Type.EDIT)
        if(metaField.id) {
            securityValidator.validateCompany(webServicesSession.getMetaField(metaField.id).entityId, Validator.Type.EDIT)
        }
    ...
    def delete () {
        ...
        if (params.id) {
            ...
            metaField = webServicesSession.getMetaField(params.int("id"))
        }
        ...
        try {
            ...
            if ( useCnt> 0){
                ...
                webServicesSession.deleteMetaField(params.int('id'));
