...
class InvoiceController {
	...
	def list () {
		...
		if (params.applyFilter || params.partial) {
			...
			if(selected){
				...
				paymentUrlLogs =new PaymentUrlLogDAS().findAllByInvoiceId(invoiceDto.getId())
			}
			checkForError(paymentUrlLogs)
			render  view: 'list',
			model: [        invoices: invoices,
				filters: filters,
				selected: selected,
	...
	def show () {
		...
		InvoiceWS invoiceWS = InvoiceBL.getWS(invoice)
		securityValidator.validateUserAndCompany(invoiceWS, Validator.Type.VIEW)
		...
		def paymentUrlLogs =new PaymentUrlLogDAS().findAllByInvoiceId(invoice.getId())
		render template: params.template ?: 'show',
		model: [          selected: invoice,
			currencies: retrieveCurrencies(),
			lines: lines,
			paymentUrlLogs: new PaymentUrlLogDAS().findAllByInvoiceId(invoice.getId()),
			availableCredits: availableCredits,
	...
	/**
	 * The generateEInvoice() method receives an invoice ID as input from invoice/_show.gsp (specifically, the ##Generate e-invoice button).
	 * It then calls the createEInvoice() method to generate an e-invoice for the entity that has provided a GST number.
	 */
	...
	def generateEInvoice() {
		...
		try {
			String msg = invoiceResourceHelperService.createEInvoice(invoiceId)
			InvoiceWS invoiceWS = InvoiceBL.getWS(InvoiceDTO.get(invoiceId))
			if (!msg.isEmpty() && msg.contains(invoiceWS.getIrn()))
				flash.message = "e.invoice.gen.success"
			else {
				log.error = "e.invoice.gen.failed"
				flash.error = "e.invoice.gen.failed"
			}
	...
	def sendPaymentLinkEmail() {
		...
		if (invoiceId) {
			...
			securityValidator.validateUserAndCompany(InvoiceBL.getWS(invoice), Validator.Type.EDIT)
			try {
				def sent = webServicesSession.notifyPaymentLinkByEmail(invoiceId)
				if (sent) {
					flash.message = 'invoice.prompt.success.email.payment.url'
					flash.args = [invoiceId]
				} else {
					flash.error = 'invoice.prompt.failure.email.payment.url'
					flash.args = [invoiceId]
				}

			} catch (SessionInternalError sie) {
				log.error("Exception occurred sending payment link email", sie)
				viewUtils.resolveException(flash, session.locale, sie)
			}
		}
	...
	def getData() {
		def invoice = InvoiceDTO.get(params.int('invoiceId'))
		Map<String, Object> map = new HashMap<>()
		map.put("totalAmount", String.valueOf(invoice.getTotal()))
		map.put("invoiceId", invoice.getId())
		def paymentUrlLogDto = webServicesSession.createPaymentUrl(map)
		render(paymentUrlLogDto)
	}

	def checkPaymentStatus() {
		def paymentUrlLogId = params.int('paymentUrlLogId')
		webServicesSession.executePaymentTask(paymentUrlLogId,null,"checkPaymentUrlStatus")
		render("send")
	}

	def cancelPaymentLink() {
		def paymentUrlLogId = params.int('paymentUrlLogId')
		webServicesSession.executePaymentTask(paymentUrlLogId,null,"cancelPaymentUrl")
		render("send")
	}

	def checkForError(List paymentUrlLogs) {
		for (PaymentUrlLogDTO paymentUrlLog : paymentUrlLogs) {
			if (paymentUrlLog.getStatus().equals(Status.FAILED)) {
				String statusResponse = paymentUrlLog.getPaymentUrlResponse()
				PaymentResponse paymentResponse = new Gson().fromJson(statusResponse, PaymentResponse.class)
				flash.error = String.valueOf(paymentResponse.getResponseMessage())
			}
		}
	}
}
