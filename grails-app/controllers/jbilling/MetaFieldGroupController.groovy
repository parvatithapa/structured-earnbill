...
class MetaFieldGroupController {
    ...
    def list (){
        ...
        securityValidator.validateCompany(selectedGroup?.entityId, Validator.Type.VIEW)
        ...
        if (params.template)
            ...
            render(view: 'listCategories', model: [selectedCategory: entityType?.name(), categories: Arrays.asList(EntityType.values()),
													groups: groups, selected: selectedGroup, listColumn:listColumn, entityType: entityType])
        }
    }
	...
	def show (){
        ...
        securityValidator.validateCompany(metaFieldGroup?.entityId, Validator.Type.VIEW)
    ...
    def edit (){
        ...
        securityValidator.validateCompany(metaFieldGroup?.entityId, Validator.Type.EDIT)
    ...
    def save (){
        ...
        if(!params?.id?.equals("0")) {
            MetaFieldGroupWS oldMfGroup = webServicesSession.getMetaFieldGroup(params.int("id"))
            securityValidator.validateCompany(oldMfGroup?.entityId, Validator.Type.EDIT)
        }
