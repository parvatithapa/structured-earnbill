...
class DiscountController {
    ...
    def show () {
        ...
        if (discount) {
            securityValidator.validateCompany(discount?.entity?.id, Validator.Type.VIEW)
        }
    ...
    @Secured(["hasAnyRole('DISCOUNT_151', 'DISCOUNT_153')"])
    def edit () {
        def discount
        ...
        if(discount.id) {
            if(SpringSecurityUtils.ifNotGranted('DISCOUNT_153')) {
                def message = """Unauthorized discount edit by caller (id ${session['user_id']})"""
                log.warn(message)
                throw new SecurityException(message);
            }
        } else {
            if(SpringSecurityUtils.ifNotGranted('DISCOUNT_151')) {
                def message = """Unauthorized discount edit by caller (id ${session['user_id']})"""
                log.warn(message)
                throw new SecurityException(message);
            }
        ...
        securityValidator.validateCompany(discount?.entityId, Validator.Type.EDIT)
    ...
    @Secured(["DISCOUNT_152"])
    def deleteDiscount () {
        int discountId = params.int('id')

        if (discountId) {
            def discount = webServicesSession.getDiscountWS(discountId)
            securityValidator.validateCompany(discount?.entityId, Validator.Type.EDIT)
    ...
    def saveDiscount () {
        ...
        if(params.'discount.id' && params.int('discount.id')) {
            def oldDiscount = webServicesSession.getDiscountWS(params.int('discount.id'))
            securityValidator.validateCompany(oldDiscount?.entityId, Validator.Type.EDIT)
            ...
            if(SpringSecurityUtils.ifNotGranted('DISCOUNT_151')) {
                def message = """Unauthorized discount create by caller (id ${session['user_id']})"""
                log.warn(message)
                throw new SecurityException(message);
            }
        }
