...
class EnumerationsController {
    ...
    def list () {
        ...
        def selected = (params.id && params.id.isNumber()) ? webServicesSession.getEnumeration(params.int('id')) : null
        ...
        def usingJQGrid = PreferenceBL.getPreferenceValueAsBoolean(company_id, Constants.PREFERENCE_USE_JQGRID);
    ...
    private def Object getEnumerationsJsonData(enums, GrailsParameterMap params) {
        ...
        def totalRecords =  webServicesSession.getAllEnumerationsCount()
    ...
    def delete () {
        def enumerationId = params.int('id')
        if (enumerationId) {
            def enumer = EnumerationDTO.get(enumerationId)
            securityValidator.validateCompany(enumer?.entityId, Validator.Type.EDIT)
    ...
    def edit () {
        ...
        securityValidator.validateCompany(enumeration?.entityId, Validator.Type.EDIT)
