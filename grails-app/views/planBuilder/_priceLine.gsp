...
<g:set var="strategy" value="${PriceModelBL.getTypeEnum(model)}"/>
<g:set var="editable" value="${index == params.int('newLineIndex')}"/>

<g:formRemote name="price-${index}-update-form" url="[action: 'edit']" update="ui-tabs-review" method="GET">
...
<ul>
    ...
    <li id="line-${index}-editor" class="editor ${editable ? 'open' : ''}">
        <div class="box">
            <div class="sub-box">
              <div class="form-columns plan-price">
                ...
                <g:if test="${model?.type}">
                    <g:render template="/priceModel/builderModel" model="[model: model, priceModelData: priceModelData, index: index]"/>
                </g:if>
            </div>
          </div>
        </div>
        ...
        <div class="btn-box">
            ...
            <g:remoteLink class="submit cancel" action="edit" params="[_eventId: 'removePrice', index: index]" update="ui-tabs-review" method="GET">
