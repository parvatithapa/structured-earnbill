...
<head>
    ...
    <style>
    %{-- This class used for EDI File Name and Type content, make in proper formate--}%
    .max-width-column {
        max-width: 180px
    }
    </style>
...
<content tag="column1">
    <g:render template="ediFilesTemplate" model="['ediFiles': ediFiles, selected: selected]"/>
</content>

<content tag="column2">
    <g:if test="${selected}">
        <g:render template="show" model="[ediFile: selected]"/>
    </g:if>
    <g:else>
        <!-- show empty block -->
