...
<g:if test="${'busy' == jobStatus}">
    ...
    <g:formRemote name="product-job-status-form" url="[controller: 'batchJob', action:'jobStatus', id: jobId]" update="product-download-box">
        <g:hiddenField name="template" value="/bulkDownload/processDownloadPricesWait"/>
    </g:formRemote>
...
<g:elseif test="${'done' == jobStatus}">
    <div><g:message code="product.download.label.done"/></div>

    <div>
        <g:link controller="batchJob" action="jobFile" id="${jobId}" params="[key: 'output_file']" >
            <g:message code="product.download.label.file"/></g:link>

        <g:if test="${executionParams?.getInt(ProductImportConstants.JOB_PARAM_ERROR_LINE_COUNT, 0) > 0}">
            <g:message code="product.download.label.error.occurred"/>
...
<g:else>
    <div><g:message code="product.download.label.error"/></div>
