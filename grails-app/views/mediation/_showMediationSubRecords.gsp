...
<%--
  ...
  @since 11-May-2018
--%>
<td class="right"><g:message code="${messageCode}"/></td>
<td class="value">${value}</td>
<g:if test="${value != 0}">
    <td class="value">
        <sec:access url="/invoice/list">
            <g:link controller="mediation" action="${action}" id="${processId}" params="${params + ['status': status, 'first': first, 'selectedCallType': selectedCallType]}">
