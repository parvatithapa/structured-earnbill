...
function addNewDescription(parent, fieldIdx) {
    ...
    removeSelectedLanguage(parent, fieldIdx);
}

function removeDescription(elm){
    removeDescription(elm, '')
}

function removeDescription(elm, fieldIdx){
...
function loadAvailableDescriptionLangs(parent, fieldIdx) {
    ...
    if (languages[0] != '') {
        ...
        $(ancestor + '#newDescriptionLanguage'+fieldIdx).each(function () {
            updateSelectLabel(this);
        });

        $(ancestor + '#newDescriptionLanguage'+fieldIdx).change(function () {
            updateSelectLabel(this);
        });
    } else {
...
function removeSelectedLanguage(parent){
    removeSelectedLanguage(parent, '')
}

function removeSelectedLanguage(parent, fieldIdx){
    var ancestor = isEmpty(parent) ?  '' : '#' + parent + ' '
    $(ancestor + '#newDescriptionLanguage'+fieldIdx+' option:selected').remove();
    if(!$(ancestor + '#newDescriptionLanguage'+fieldIdx+' option').size()){
        $(ancestor + '#addDescription'+fieldIdx).hide();
    }
}
