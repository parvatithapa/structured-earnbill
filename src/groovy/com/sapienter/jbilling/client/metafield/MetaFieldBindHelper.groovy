...
class MetaFieldBindHelper {
    ...
    static def List<MetaFieldValueWS> bindMetaFields(Collection<MetaField> metaFields, GrailsParameterMap params) {
        ...
        metaFields.each{
            ...
            /*We have show functionalliy of the json object on the view. Now json object will show as key value pair on the
            * screen where only value will be editable.So new code for binding JSON_OBJECT*/
            if(it.dataType==DataType.JSON_OBJECT){
                bindJSONObject(fieldValue, params, "JSON_metaField_${it.id}")
            }else{
                bindData(fieldValue, params, "metaField_${it.id}")
            }
            if (!fieldValue.getValue() && !it.getDataType().equals(DataType.DECIMAL)) {
            ...
            if (((metaFieldWS == null && it.isMandatory()) || (metaFieldWS !=null && metaFieldWS.getValue()==null)) && it.getDataType().equals(DataType.BOOLEAN)) {
    ...
    static def List<MetaFieldValueWS> bindMetaFieldsWithGroupId(Collection<MetaField> metaFields, GrailsParameterMap params) {
        List<MetaFieldValueWS> fieldsArray = new LinkedList<MetaFieldValueWS>();
        metaFields.each{
            def fieldValue = it.createValue();
            bindData(fieldValue, params, "metaField_${it.id}")
            ...
            if(it?.metaFieldGroups){
                metaFieldWS.setGroupId(it?.metaFieldGroups?.first()?.id)
            }
        };
    ...
    static def MetaFieldWS bindMetaFieldName(MetaFieldWS metaField, GrailsParameterMap params, boolean swallowException, String index=''){
        ...
        if(params['dependentMetaFields'+index]){
            def dependencyList = params.list('dependentMetaFields'+index)
            Integer[] dependencyMetaFields = new Integer[dependencyList.size()]
            dependencyList.eachWithIndex { def metaFieldId, Integer idx ->
                dependencyMetaFields[idx] =  metaFieldId as Integer
            }
            metaField.setDependentMetaFields(dependencyMetaFields)
        }
        ...
        if (params['defaultValue'+index]) {
            ...
            if (defaultValue == null || defaultValue.getValue() == null) {
                ...
                defaultValue.getMetaField().setDataType(metaField.dataType)
    ...
    private static def bindJSONObject(MetaFieldValue model,GrailsParameterMap modelParams, String prefix) {
        Map<String, String> map=new LinkedHashMap<>()
        List<String> keys=modelParams.list(prefix+"_name");
        List<String> values=modelParams.list(prefix+"_value");
        ...
        model.setValue(new JsonBuilder(map).toPrettyString())
    }
    ...
    /*This method convert JSON string to Map */
    public static Map<String, String> convertJSONStringToMap(String jsonString){
        ...
        try{
            resultMap = mapperObj.readValue(jsonString, new TypeReference<HashMap<String,String>>(){});
        }catch (Exception e){
