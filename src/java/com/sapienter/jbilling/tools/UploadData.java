...
*/
            ...
            while (record != null) {
                ...
                if (credit_card_number >= 0 &&
                    ...
                    instrument = piBl.getCreditCardObject(fields[credit_card_number].trim().toCharArray(), company);
                    Date date = new Date();
                    if (expiry_month >= 0) {
                    ...
                    piBl.updateCharMetaField(instrument, DateTimeFormat.forPattern(com.sapienter.jbilling.server.util.Constants.CC_DATE_FORMAT).print(date.getTime()).toCharArray(), MetaFieldType.DATE);
                    if (name_on_card >= 0) {
                        piBl.updateCharMetaField(instrument, (fields[name_on_card].trim()).toCharArray(), MetaFieldType.TITLE);
                    }
