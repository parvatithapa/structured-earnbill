...
public class RateCardBeanFactory {
    ...
    public AbstractBeanDefinition getLoaderBeanDefinition(String readerBeanName) {
        ...
        beanDef.addPropertyValue("indexColumnNames", "match_column");
    ...
    public RateCardFinder getFinderInstance() {
        if(Context.getApplicationContext().containsBean(getFinderBeanName())) {
            return Context.getBean(getFinderBeanName());
        }
        return doRegisterAndGetFromContext();
    }

    /**
     * Register RateCard in application context and retrieved from Context.
     * method is useful for cluster environment where one instance has that bean in application context but other
     * instance application bean does not have it in application context.
     * @return
     */
    private RateCardFinder doRegisterAndGetFromContext() {
