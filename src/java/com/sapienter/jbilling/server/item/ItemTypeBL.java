...
public class ItemTypeBL {
    ...
    public void create(ItemTypeDTO dto) {
        ...
        itemType.setInternal(dto.isInternal());
    ...
    public void delete(Integer executorId) {
        if (isInUse()) {
            throw new SessionInternalError("Cannot delete a non-empty item type, remove items before deleting.",
                                            new String[]{"validation.error.category.is.not.empty"});
        }
        ...
        if(itemType.isGlobal()){
            if(itemType.getEntity() != null) {
                ...
                for(Integer id: allEntities){
                    itemType.updateMetaFieldsWithValidation(dto.getEntity().getLanguageId(), id, accountTypeId, dto);
                }
            }
            ...
            for(CompanyDTO companyDTO : dto.getEntities()) {
                itemType.updateMetaFieldsWithValidation(companyDTO.getLanguageId(), companyDTO.getId(), accountTypeId, dto);
            }
        }else {
            // if not global category and caller company is not root then set only meta-fields visible to the caller company
            itemType.updateMetaFieldsWithValidation(new CompanyDAS().find(callerCompanyId).getLanguageId(), callerCompanyId, accountTypeId, dto);
        }
    }
	...
	public static ItemTypeWS toWS(ItemTypeDTO dto){
        ...
        ws.setOnePerCustomer(dto.isOnePerCustomer());
        ws.setOnePerOrder(dto.isOnePerOrder());
		...
		for(AssetStatusDTO statusDTO : dto.getAssetStatuses()) {
			if(statusDTO.getDeleted() == 0) {
				ws.getAssetStatuses().add(new AssetStatusDTOEx(
						...
						statusDTO.getIsPending(),
                        statusDTO.getIsOrderFinished()));
			}
		}

        ws.getAssetMetaFields().addAll(dto.getAssetMetaFields().stream()
                                                               .map(MetaFieldBL::getWS)
                                                               .collect(Collectors.toList()));

        ws.setMetaFields(dto.getMetaFields().stream()
                                            .map(MetaFieldBL::getWS)
