...
public class TelcoUsageManagerTask extends RulesBaseTask implements IItemPurchaseManager {

    // for the rules task, needed due to some class hierarchy problems further up
    // todo: remove when all rules plug-ins are deleted.
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(TelcoUsageManagerTask.class));
        ...
        boolean isFound = false;
        	if(null != records && !records.isEmpty() ) {
                for (PricingField pricingField : records.get(0).getFields()) {
                    if (null != pricingField && pricingField.getName().equals(dnisFieldName)) {
                       isFound = true;
    ...
    protected OrderLineDTO getOrderLine(Integer itemId, Integer languageId, Integer userId, Integer currencyId,
        ...
        // Setting the DNIS value in the OrderLine as a call Identifier
