...
public class RulesPricingTask2 extends RulesBaseTask implements IPricing {
    ...
    @Override
    public BigDecimal getPrice(PriceContextDTO priceContext, BigDecimal defaultPrice, OrderDTO pricingOrder, OrderLineDTO orderLine, boolean singlePurchase) throws TaskException {

        // the result goes in the memory context
        ItemDTO item = priceContext.getItem();
        Integer userId = priceContext.getUserId();
        Integer currencyId = priceContext.getCurrencyId();
        List<PricingField> fields = priceContext.getFields();
        
        PricingResult result = new PricingResult(item.getId(), priceContext.getQuantity(), userId, currencyId);
