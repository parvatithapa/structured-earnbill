...
public class PlanItemBundleBL {
    ...
    public static PlanItemBundleWS getWS(PlanItemBundleDTO dto) {
    	...
    	PlanItemBundleWS ws = new PlanItemBundleWS();
    	ws.setId(dto.getId());
        ws.setAddIfExists(dto.addIfExists());

        ws.setQuantity(dto.getQuantity());

        if (dto.getPeriod() != null) ws.setPeriodId(dto.getPeriod().getId());
        if (dto.getTargetCustomer() != null)ws.setTargetCustomer(dto.getTargetCustomer().name());
        return ws;
