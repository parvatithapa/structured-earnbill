...
public class AssetStatusBL {
     ...
     * Convert a collection of AssetStatusDTOEx into AssetStatusDTO
     *
     * @param dtoExes, languageId
     * @return
     * @throws SessionInternalError
     */
    public Set convertAssetStatusDTOExes(Collection<AssetStatusDTOEx> dtoExes, Integer languageId) throws SessionInternalError {
        AssetStatusBL statusBL = new AssetStatusBL();

        HashSet<AssetStatusDTO> dtos = new HashSet<AssetStatusDTO>((int)(dtoExes.size() * 1.5));
        for(AssetStatusDTOEx dtoEx : dtoExes) {
            AssetStatusDTO dto = new AssetStatusDTO(dtoEx, languageId);
            dtos.add(dto);
        }
        return dtos;
    }
    ...
    public AssetStatusDTO mergeBasicProperties(AssetStatusDTO destination, AssetStatusDTO source) {
        ...
        destination.setIsPending(source.getIsPending());
        destination.setIsActive(source.getIsActive());
        destination.setIsOrderFinished(source.getIsOrderFinished());
    ...
    public static final AssetStatusDTOEx getWS(AssetStatusDTO dto) {
        return new AssetStatusDTOEx(dto.getId(), dto.getDescription(), dto.getIsDefault(), dto.getIsAvailable(), dto.getIsOrderSaved(), dto.getIsInternal(), dto.getIsActive(), dto.getIsPending(), dto.getIsOrderFinished());
    }
     ...
     * @return
     */
    public AssetStatusDTO findOrderFinishedStatusForItem(int id) {
        return das.findOrderFinishedStatusForItem(id);
    }
    ...
    public AssetStatusDTO getMemberOfGroupStatus() {
        return das.find(Constants.ASSET_STATUS_MEMBER_OF_GROUP);
    }
     ...
     * @return
     */
    public AssetStatusDTO findOrderSavedStatusForItem(int id) {
        return das.findOrderSavedStatusForItem(id);
    }
     ...
     * @return
     */
    public AssetStatusDTO findActiveStatusForItem(int id) {
        return das.findActiveStatusForItem(id);
    }
     ...
     * @return
     */
    public AssetStatusDTO findPendingStatusForItem(int id) {
        return das.findPendingStatusForItem(id);
    }
}
