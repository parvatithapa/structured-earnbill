...
public class PlanItemDAS extends AbstractDAS<PlanItemDTO> {
    ...
    public long findPlanItemsCountByItemId(int itemId) {
        Query query = getSession().getNamedQuery("PlanItemDTO.findCountByItemId")
                .setInteger("itemId", itemId);
        return ((Long)query.uniqueResult()).longValue();
    }
    
    public boolean isPlan(Integer id) {
        Query query = getSession().createSQLQuery("SELECT * FROM plan where item_id = (SELECT item_id FROM plan_item WHERE id = :id)");
