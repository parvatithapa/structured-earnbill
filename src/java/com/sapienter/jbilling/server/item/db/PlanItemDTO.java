...
@NamedQueries({
        @NamedQuery(name = "PlanItemDTO.findCountByItemId", query = "select count(planItem.id) "
                + " from PlanItemDTO planItem where planItem.item.id = :itemId")
})
// todo: cache config
public class PlanItemDTO implements Serializable, Auditable {
