...
public class ItemTypeDAS extends AbstractDAS<ItemTypeDTO> {
    ...
    public ItemTypeDTO getCreateInternalPlansType(Integer entityId) {
        ...
        criteria.add(Restrictions.eq("entity.id", entityId))
                .add(Restrictions.eq("description", Constants.PLANS_INTERNAL_CATEGORY_NAME))
        ...
        if (type == null) {
            ...
            type.setAllowAssetManagement(0);
            type.setDescription(Constants.PLANS_INTERNAL_CATEGORY_NAME);
            type.setInternal(true);
            type.setOrderLineTypeId(Constants.ORDER_LINE_TYPE_ITEM);
            type.setEntity(new CompanyDTO(entityId));
            type = save(type);

            Set<CompanyDTO> e = new HashSet<CompanyDTO>();
            e.add(new CompanyDTO(entityId));
            type.setEntities(e);
        }
...
//                itemTypeCriteria.add(Subqueries.exists(existCriteria));
    ...
    public List<ItemTypeDTO> findItemCategories(Integer entity, List<Integer> entities, boolean isRoot) {
        //added the left join in the query for fetching global type category.
        Criteria criteria = getSession().createCriteria(ItemTypeDTO.class)
                .createAlias("entities", "entities", CriteriaSpecification.LEFT_JOIN);
