...
public class ItemTypeDTO extends AbstractDescription implements MetaContent, Serializable, Auditable {
    ...
    public AssetStatusDTO findOrderSavedStatus() {
        for( AssetStatusDTO assetStatusDTO : assetStatuses) {
            //add this condition as well if necessary  || 0 == assetStatusDTO.getIsPending()
            if(assetStatusDTO.getDeleted() == 0 && assetStatusDTO.getIsOrderSaved() == 1 && 1 == assetStatusDTO.getIsActive()) return assetStatusDTO;
        }
        return null;
    }
    ...
    public void updateMetaFieldsWithValidation(Integer languageId, Integer entitId, Integer accountTypeId, MetaContent dto) {
        MetaFieldHelper.updateMetaFieldsWithValidation(languageId, entitId, accountTypeId, this, dto, this.isGlobal());
    }
