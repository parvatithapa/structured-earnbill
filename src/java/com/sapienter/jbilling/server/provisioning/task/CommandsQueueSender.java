...
public class CommandsQueueSender {
    ...
    public void postProvisioningCommand(final LinkedList<ProvisioningCommandDTO> commands, final String eventType) throws JMSException {
        LOG.debug("calling postProvisioningCommand()");

        if (commands == null) {
            LOG.debug("Found NULL command Object");
        ...
        for (Iterator<ProvisioningCommandDTO> it = commands.iterator(); it
                .hasNext();) {
            final ProvisioningCommandDTO command = it.next();
            ...
            jmsTemplate.send(destination, new MessageCreator() {
                public Message createMessage(Session session)
                        throws JMSException {
                    MapMessage message = session.createMapMessage();
                    command.postCommand(message, eventType);
                    return message;
                }
            });
        }
