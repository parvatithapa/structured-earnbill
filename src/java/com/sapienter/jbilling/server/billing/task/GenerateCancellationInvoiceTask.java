...
public class GenerateCancellationInvoiceTask extends AbstractCronTask {
	...
	private void sendJmsMessage(CancellationRequestDTO cancellationRequest) {
		...
		jmsTemplate.send(destination, session -> {
			UserDTO user = cancellationRequest.getCustomer().getBaseUser();
			...
			objectMessage.setObjectProperty(Constants.CANCELLATION_USER_ENTITY_ID, user.getCompany().getId());
	...
	private void sendFreeTrialPlanJmsMessage(Integer userId) {
		...
		jmsTemplate.send(destination, session -> {
			...
			ObjectMessage objectMessage = session.createObjectMessage();
			objectMessage.setObject(userId);
			objectMessage.setObjectProperty(Constants.CANCELLATION_USER_ENTITY_ID, getEntityId());
			return objectMessage;
		});
	}
}
