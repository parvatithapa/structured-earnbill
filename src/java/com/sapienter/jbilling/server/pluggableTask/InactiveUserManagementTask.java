...
public class InactiveUserManagementTask extends AbstractCronTask {
    ...
    public InactiveUserManagementTask() {
    ...
    public void doExecute (JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        List<Integer> inActiveUserIdsList = getUsersInactiveSince(activityThresholdDate);

        if (null != inActiveUserIdsList && !inActiveUserIdsList.isEmpty()) { // means there are users that have lastLogin before activityThresholdDate
            LOG.debug("Number of inactive users since " + activityThresholdDate + " = " + inActiveUserIdsList.size());
        } else {
        ...
        for (Integer inactiveUserId : inActiveUserIdsList) {
            UserDTOEx userDTOEx = userSessionBean.getUserDTOEx(inactiveUserId);
            userSessionBean.updateUserAccountExpiryStatus(userDTOEx, true);
        }
    }

    private List<Integer> getUsersInactiveSince(Date activityThresholdDate) {
        UserDAS userDas = new UserDAS();
        try {
            List<Integer> inactiveUserList = userDas.findUsersInActiveSince(activityThresholdDate, getEntityId());
