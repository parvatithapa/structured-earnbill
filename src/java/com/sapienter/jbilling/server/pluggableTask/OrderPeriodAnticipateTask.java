...
public class OrderPeriodAnticipateTask extends BasicOrderPeriodTask {
    ...
    public Date calculateEnd(OrderDTO order, Date processDate,
                             int maxPeriods, Date periodStarts, Date viewLimit)
        ...
        this.viewLimit = getViewLimit(order.getUser().getId(), processDate);

        if (order.getAnticipatePeriods() != null &&
                order.getAnticipatePeriods().intValue() > 0) {
            try {
                GregorianCalendar cal = new GregorianCalendar();
                cal.setTime(this.viewLimit);
                ...
                LOG.debug("Ant periods:" + order.getAnticipatePeriods() + " " +
                        "view limit: " + this.viewLimit + " extended " + cal.getTime());
                this.viewLimit = cal.getTime();
            } catch (Exception e) {
                throw new TaskException(e);
            } 
        } 
        
        return super.calculateEnd(order, processDate, maxPeriods, periodStarts, null);
    }
}
