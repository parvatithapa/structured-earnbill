...
//        }
    ...
    private Result doFakeAuthorization(PaymentDTOEx payment, String transactionId) throws PluggableTaskException {
    	...
    	if(piBl.isCreditCard(payment.getInstrument())) {
    		resultId = getProcessResultId(piBl.getCharMetaFieldByType(payment.getInstrument(), MetaFieldType.PAYMENT_CARD_NUMBER));
     ...
     */
    private Integer getProcessResultId(char[] number) {
        char last = (number.length == 0) ? ' ' : number[(number.length - 1)];
    ...
    private static class NameStartsWithFilter implements Filter {
        ...
        public boolean accept(PaymentInformationDTO card) {
        	...
        	char[] name = new PaymentInformationBL().getCharMetaFieldByType(card, MetaFieldType.TITLE);

            if(name != null){
                for(int i =0; i< myPrefix.length() && i<name.length; i++){
                    if(name[i] == myPrefix.charAt(i)){
                        returnValue = true;
                    }
                    else{
                        returnValue = false;
