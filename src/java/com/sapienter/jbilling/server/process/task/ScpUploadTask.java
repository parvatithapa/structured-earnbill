...
public class ScpUploadTask extends AbstractCronTask {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        logger.debug("Scanning {} {} for files matching {}",path.getPath(),(recursive ? " recursively" : ""), fileRegex);
        ...
        for (File file : path.listFiles()) {
            if (file.isDirectory()) {
                ...
                if (!uploadMarker.exists()) {
                    ...
                    logger.debug("Found {} for upload.", files.size());
    ...
    public void upload(List<File> files, String remotePath, String host, String userName, String password )
        ...
        logger.debug("Uploading {} files to {}@{}:{}", files.size(), userName, host, remotePath);
    ...
    public void mark(List<File> files, File uploadMarker) throws JobExecutionException {
        logger.debug("Marking folder {} as uploaded. Writing {}", uploadMarker.getParentFile().getPath(),
            uploadMarker.getName());

        FileWriter writer = null;
        try {
            writer = new FileWriter(uploadMarker, true);
            writer.write("Uploaded " + TimezoneHelper.serverCurrentDate() + "\n");
            for (File file : files)
