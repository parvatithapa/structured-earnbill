...
public abstract class AbstractChargeTask extends PluggableTask implements InvoiceCompositionTask {
    ...
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(AbstractChargeTask.class));
    ...
    protected void applyCharge(NewInvoiceContext invoice, Integer userId, BigDecimal taxOrPenaltyBaseAmt, Integer INVOICE_LINE_TYPE) {
        ...
        PriceModelDTO taxItemPrice = taxItem.getPrice(companyCurrentDate(), invoice.getEntityId());
        if (taxItemPrice.getType()==PriceModelStrategy.LINE_PERCENTAGE) {
            LOG.debug("Percentage: " + taxItemPrice.getRate());
            LOG.debug("Calculating tax on = " + taxOrPenaltyBaseAmt);
            taxOrPenaltyValue = taxOrPenaltyBaseAmt.multiply(taxItemPrice.getRate()).divide(
                    BigDecimal.valueOf(100L), Constants.BIGDECIMAL_SCALE, Constants.BIGDECIMAL_ROUND);
            taxOrPenaltyRate= taxItemPrice.getRate();
        } else {
            LOG.debug("Flat Price.");
            ItemBL itemBL = new ItemBL(taxItem);
            taxOrPenaltyValue = itemBL.getPriceByCurrency(invoice.getCreateDatetime(), taxItem, userId, invoice.getCurrency().getId(), null, null);
    ...
    protected BigDecimal calculateAndApplyTax(NewInvoiceContext invoice, Integer userId) {
        ...
        if (taxItem.getPrice(companyCurrentDate(), invoice.getEntityId()).getType()==PriceModelStrategy.LINE_PERCENTAGE) {
