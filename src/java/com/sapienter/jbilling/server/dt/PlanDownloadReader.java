 ...
 */
public class PlanDownloadReader implements ItemReader<PlanDTO>, ItemStreamReader<PlanDTO>, StepExecutionListener {
    ...
    private List<PlanDTO> allPlansByEntityId;
    ...
    public void beforeStep(StepExecution stepExecution) {
        ...
        planNumber = jobParameters.getString(ProductImportConstants.JOB_PARAM_IDENTIFICATION_CODE);
    ...
    @Override
    public PlanDTO read() throws Exception {
        try (RunAsUser ctx = new RunAsCompanyAdmin(entityId)) {

            try {
                if (StringUtils.isNotBlank(planNumber) && !itemProcessed) {
                    if ((new PlanDAS().findPlanByPlanNumber(planNumber, entityId)).size() > 1) {
                        throw new SessionInternalError("Plan Number should be unique",
                                new String[] { "Plan number should be unique :" + planNumber });
                    }
                    PlanDTO plan = new PlanDAS().findPlanByPlanNumber(planNumber, entityId).get(0);
                    itemProcessed = true;

                    return plan;

                } else if (allPlansByEntityId.get(0) != null && arrayIndex < allPlansByEntityId.size()) {

                    PlanDTO plan = allPlansByEntityId.get(arrayIndex++);
                    return plan;
                }
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException {
        ...
        if(StringUtils.isBlank(planNumber)) {
            try (RunAsUser ctx = new RunAsCompanyAdmin(entityId)) {
                allPlansByEntityId = new PlanDAS().findAllActive(entityId);
            }
        }
    }
