 ...
 */
public class ProductCustomerPriceFileReader implements ItemStreamReader, ItemReader, StepExecutionListener {
        ...
        try {
            while (!recordFinished) {
                ...
                if (peakNextItem() == null) {
                    logger.debug("Bulk Upload - Customer Level: FILE FINISHED - {}", fileName);
                    recordFinished = true;
                    continue;
                }
                ...
                try {
                    ...
                    logger.error("Bulk Upload - Customer Level: Price FINISHED with ERROR - {}", fileName);
                ...
                if (peakNextItem() == null || (!peakNextItem().getLine().isEmpty() &&
                    ...
                    logger.debug("Bulk Upload - Customer Level: RECORD FINISHED - {}", fileName);
            ...
            {
                logger.error(String.format("Bulk Upload - Customer Level - Row contains unsupported tags/info: %s", currentRow),
                        exceptionMessage);
                exceptionMessage = "Invalid Row format";
            }
            writeLineToErrorFile(currentRow, exceptionMessage);

            logger.error(String.format("Bulk Upload - Customer Level: %s", currentRow), exceptionMessage);
    ...
    private void process(WrappedObjectLine<FieldSet> wrappedObjectLine) throws Exception {
        ...
        try {
            ...
            //If the line identifier is PRICE, then that means the Customer type price is starting so create a new instance
            ...
            if (lineId.equals(ProductImportConstants.ColumnIdentifier.PRICE.toString())) {
                ...
                logger.debug("Bulk Upload - Customer Level: STARTING NEW Customer TYPE PRICE from file - {}", fileName);
                ...
                currencyId = ProductUploadFileItemProcessor.processCustomerTypePrice(fieldSet, fileItem, entityId, userId);
            }
