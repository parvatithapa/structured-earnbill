...
public class BasicLoaderImpl implements ILoader {
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(BasicLoaderImpl.class));
    ...
    protected void initDB() {
        LOG.debug("Initializing cached memory table '%s'", getTableName());
        ...
        if (reader.validate(new ArrayList<String>())) {
            try {
                ...
                for (List<CallDataRecord> group : reader) {
                    LOG.debug("Loading %s records.", group.size());
                    ...
                    final List<CallDataRecord> batch = new ArrayList<CallDataRecord>(group);
        ...
        watch.stop();
        LOG.debug("Finished loading cached memory table in (secs): %s", watch.getTotalTimeSeconds());
    ...
    private boolean createIndexes() {
        ...
        LOG.debug("Schema Index: '%s'", indexSql);
        this.jdbcTemplate.execute(indexSql.toString());
        return true;
    }

    private String[] computeInsertSql(List<CallDataRecord> records) {
     ...
     * @return
     */
    private boolean createTable(CallDataRecord record) {
        ...
        String dropTableIfExists = "DROP TABLE IF EXISTS " + getTableName();
        LOG.debug("Schema DDL: '" + dropTableIfExists + "'");
        this.jdbcTemplate.execute(dropTableIfExists);

        // execute create table SQL
        LOG.debug("Schema DDL: '%s'", createTable);
