 ...
 *
 * Unlike the the {@link com.sapienter.jbilling.server.mediation.task.AbstractJDBCReader} this reader does not update or fetch the mediation "last ID read"
...
public class StatelessJDBCReader extends AbstractJDBCReader {
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(StatelessJDBCReader.class));
     ...
     */
    @Override
    protected void recordRead(final CallDataRecord record, final int[] keyColumnIndexes) {
     ...
     */
    @Override
    protected void batchRead(final List<CallDataRecord> records, final int[] keyColumnIndexes) {
    }
}
