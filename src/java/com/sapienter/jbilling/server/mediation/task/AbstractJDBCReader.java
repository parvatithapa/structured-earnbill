...
/**
 * AbstractJDBCReader provides a generic base for all JDBC  classes.
 *
 * @see JDBCUtils
 * 
...
public abstract class AbstractJDBCReader extends AbstractReader {
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(AbstractJDBCReader.class));
    ...
    protected Integer readLastId() {
        try {
            lastId = PreferenceBL.getPreferenceValueAsInteger(
            		getEntityId(), Constants.PREFERENCE_MEDIATION_JDBC_READER_LAST_ID);
    ...
    protected void flushLastId() {
        ...
        new PreferenceBL().createUpdateForEntity(getEntityId(),
    ...
    public class Reader implements Iterator<List<CallDataRecord>> {
        ...
        protected Reader(JdbcTemplate jdbcTemplate) {
            this.initRowSet(jdbcTemplate);
        }

        private void initRowSet (JdbcTemplate jdbcTemplate) {
            jdbcTemplate.setFetchSize(getBatchSize());
            rs = jdbcTemplate.queryForRowSet(getSqlQueryString());
            parseMetaData(rs);
        }
        ...
        public List<CallDataRecord> next() {
            ...
            return new ArrayList<CallDataRecord>(records);
        }

        private List<CallDataRecord> getNextBatch() {
            // fetch records
            try {
                List<CallDataRecord> records = new ArrayList<CallDataRecord>(getBatchSize());
                int counter = 0;
     ...
     */
    protected abstract void recordRead(final CallDataRecord record, final int[] keyColumnIndexes);
     ...
     * @param keyColumnIndexes index of record PricingFields that represent key columns.
     */
    protected abstract void batchRead(final List<CallDataRecord> records, final int[] keyColumnIndexes);
