...
public abstract class AbstractFileReader extends AbstractReader {
    ...
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(AbstractFileReader.class));
    ...
    public boolean validate(List<String> messages) {
        ...
        // optionals
        directory = directory != null ? directory : getParameter(PARAMETER_DIRECTORY.getName(), Util.getSysProp("base_dir") + "mediation");
        ...
        suffix = suffix != null ? suffix : getParameter(PARAMETER_SUFFIX.getName(), "ALL");
        rename = getParameter(PARAMETER_RENAME.getName(), false);
        dateFormat = DateTimeFormat.forPattern(getParameter(PARAMETER_DATE_FORMAT.getName(), "yyyyMMdd-HHmmss"));
        ...
        LOG.debug("Started with " + " directory: " + directory + " suffix " + suffix + " rename " +
                rename + " date  format " + getParameter(PARAMETER_DATE_FORMAT.getName(), "yyyyMMdd-HHmmss") + " removeQuote " + removeQuote +
                " autoID " + autoID);
    ...
    @Override
    public Iterator<List<CallDataRecord>> iterator() {
        try {
            return new Reader();
        } catch (Exception e) {
     ...
     */
    public class Reader implements Iterator<List<CallDataRecord>> {
        private final FormatLogger LOG = new FormatLogger(Logger.getLogger(Reader.class));
        private File[] files = null;
        private int fileIndex = 0;
        private List<CallDataRecord> records = null;
        ...
        protected Reader() throws FileNotFoundException, IOException, SAXException {
            ...
            if (!nextReader()) {
                ...
                format = getFormat();
                counter = 0;
                records = new ArrayList<CallDataRecord>(getBatchSize());
            }
        }
         ...
         */
        public List<CallDataRecord> next() {
            if (records.size() == 0) {
        ...
        private CallDataRecord convertLineToRecord(String line) {
        ...
        private boolean nextReader() throws FileNotFoundException {
            ...
            if (files != null && files.length > fileIndex) { // any more to process ?
                if (bufferSize > 0) {
    ...
    /**
     * Overrides reader's target directory and file suffix
     *
     * @param directory
     * @param suffix
     */
    public void overrideFileReaderParameters(String directory, String suffix) {
