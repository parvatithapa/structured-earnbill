...
public class MigrateCustomerAccounts extends AbstractCustomSqlChange {
    ...
    private List<SqlStatement> generateMetaFieldStatements(
        ...
        statements.add(buildMFCustomerStm(customerId, aitId));
    ...
    private InsertStatement buildMFInsertStm(String fieldName, Integer aitId, Integer companyId, Object value){
        ...
        return buildInsertStatement("meta_field_value")
                ...
                .addColumnValue("id", new StringBuffer("coalesce((select max(mv.id) + 1 from meta_field_value mv),1)"));
    }
    ...
    private InsertStatement buildMFCustomerStm(Integer customerId, Integer ait){
        //string buffer is used to trick LB not to quote the string
        return buildInsertStatement("customer_account_info_type_timeline")
                .addColumnValue("id", new StringBuffer("coalesce((select max(id) + 1 from customer_account_info_type_timeline),1)"))
                .addColumnValue("customer_id", customerId)
                .addColumnValue("account_info_type_id", ait)
                .addColumnValue("meta_field_value_id", new StringBuffer("coalesce((select max(mv.id) from meta_field_value mv),1)"))
                .addColumnValue("effective_date", "19700101");
    }
