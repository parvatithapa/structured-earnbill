...
public class UsageBL {
    ...
    public void calculateMonthlyUsagePeriod(OrderDTO periodOrder) {
        ...
        try {
            ...
            Date cycleEndDate = periodTask.calculateEnd(periodOrder, TimezoneHelper.companyCurrentDate(entityId), periods, cycleStartDate, null);
    ...
    public void calculateOneTimeUsagePeriod(OrderDTO periodOrder, MainSubscriptionDTO mainSubscription) {
        ...
        Date cycleStartDate = Util.truncateDate(calendar.getTime());
    ...
    private String getCacheKey() {
        if (workingOrder != null) {
            if(workingOrder.getId() == null){
                return "user " + userId + " nullOrder " + TimezoneHelper.serverCurrentDate().getTime() + " periods " + periods;
            }else{
    ...
    public Date getPeriodEnd() {
        ...
        // end of day today
        Date today = Util.truncateDate(TimezoneHelper.companyCurrentDatePlusTemporalUnit(new UserDAS().getUserCompanyId(usagePeriod.getPeriodOrder().getUserId()), 1, ChronoUnit.DAYS));

        if (Util.truncateDate(end.getEnd()).before(today)) {
            return today;
        } else {
            return end.getEnd();
        }
    ...
    public Usage getItemUsage(Integer itemId, boolean forCompany) {
        ...
        if (getPeriodOrder() != null) {
            ...
            usage = usageDas.findUsageByItem(null, itemId, userId, userIds, com.sapienter.jbilling.server.util.Util.getEpochDate(), TimezoneHelper.companyCurrentDateByUserId(userId));
            if (usage == null) {
