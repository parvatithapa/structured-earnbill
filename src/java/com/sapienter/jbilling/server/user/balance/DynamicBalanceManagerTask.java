...
public class DynamicBalanceManagerTask extends PluggableTask implements IInternalEventsTask {
    ...
    private static final Class<Event> events[] = new Class[]{
            ...
            ReservationReleasedEvent.class,
    ...
    private BigDecimal determineAmount (Event event) {
        if (event instanceof PaymentSuccessfulEvent) {
            ...
            if (!Constants.PAYMENT_RESULT_SUCCESSFUL.equals(payment.getPayment().getResultId())
                    && !Constants.PAYMENT_RESULT_ENTERED.equals(payment.getPayment().getResultId())) {
        ...
        } else if (event instanceof ReservationCreatedEvent) {
            ReservationCreatedEvent rce = (ReservationCreatedEvent) event;
            return rce.getReservation().getAmount().negate();
        } else if (event instanceof ReservationReleasedEvent) {
            ReservationReleasedEvent rre = (ReservationReleasedEvent) event;
            return rre.getReservation().getAmount();
        } else if (event instanceof InvoiceDeletedEvent) {
        	InvoiceDeletedEvent ide = (InvoiceDeletedEvent) event;
        	BigDecimal returnAmount = BigDecimal.ZERO;
        	for(InvoiceLineDTO invocieLine : ide.getInvoice().getInvoiceLines()) {
        		if(!com.sapienter.jbilling.server.util.Constants.INVOICE_LINE_TYPE_ITEM_ONETIME.equals (invocieLine.getTypeId())) {
        			returnAmount = returnAmount.add(invocieLine.getAmount());
        		} 
        	}
        	return returnAmount; 
        } else {
    ...
    private int determineUserId (Event event) {
        if (event instanceof PaymentSuccessfulEvent) {
            ...
            ReservationCreatedEvent rce = (ReservationCreatedEvent) event;
            return rce.getReservation().getSession().getBaseUser().getId();
        } else if (event instanceof ReservationReleasedEvent) {
            ReservationReleasedEvent rre = (ReservationReleasedEvent) event;
            return rre.getReservation().getSession().getBaseUser().getId();
        } else if (event instanceof InvoiceDeletedEvent) {
        	InvoiceDeletedEvent ide = (InvoiceDeletedEvent) event;
            return ide.getInvoice().getUserId();
        } else {
