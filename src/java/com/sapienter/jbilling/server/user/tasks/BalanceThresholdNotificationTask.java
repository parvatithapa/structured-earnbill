...
public class BalanceThresholdNotificationTask extends PluggableTask implements
    ...
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(BalanceThresholdNotificationTask.class));
    ...
    private BigDecimal retrieveUserThreshold(UserDTO user) throws PluggableTaskException {
        ...
        //The threshold amount is stored in notificationBelowThreshold
        if (user.getCustomer() != null && user.getCustomer().getLowBalanceThreshold() != null
                && user.getCustomer().getLowBalanceThreshold().compareTo(BigDecimal.ZERO) > 0) {
            return new BigDecimal(user.getCustomer().getLowBalanceThreshold().toString());
        }
