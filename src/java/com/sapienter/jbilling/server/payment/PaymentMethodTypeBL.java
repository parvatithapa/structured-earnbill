...
public class PaymentMethodTypeBL {
	...
	public void setPaymentMethodType (Integer id) {
        paymentMethodType = pmtDAS.findNow(id);
	}
	...
	public PaymentMethodTemplateWS getWS(PaymentMethodTemplateDTO dto, Integer entityId) {
		...
		for(MetaField mf : dto.getPaymentTemplateMetaFields()) {
			if(mf.getEntityId() == entityId.intValue()) {
	...
	public PaymentMethodTypeWS getWS() {
		...
		ws.setOwningEntityId(paymentMethodType.getEntity().getId());
	...
	private List<Integer> getAllAccountTypes(Integer entityId){
		List<Integer> accountTypes=new ArrayList<Integer>();
		for(AccountTypeDTO accountType : new AccountTypeDAS().findAll(entityId)){
			accountTypes.add(accountType.getId());
		}
		return accountTypes;
	}
	
	public static List<MetaField> getFilteredMetaFields(PaymentMethodTypeDTO typeDTO, List<MetaFieldType> typesToExclude){
		List<MetaField> metaFields = new ArrayList<MetaField>();
		for(MetaField field: typeDTO.getMetaFields()){
			if(field.getFieldUsage() != null){
				boolean include = true;
				for(MetaFieldType type: typesToExclude){
					if(field.getFieldUsage() == (type)){
						//exclude this field
						include = false;
						break;
					}
	...
	public List<PaymentMethodTypeDTO> getAllPaymentMethodTypes(Integer entityId){
	    List<PaymentMethodTypeDTO> paymentMethodTypeDTOs = pmtDAS.getAllPaymentMethodTypes(entityId);
	    if (paymentMethodTypeDTOs.isEmpty()) {
	        return null;
	    }
