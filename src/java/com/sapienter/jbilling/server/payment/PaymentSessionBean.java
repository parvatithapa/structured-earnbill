...
public class PaymentSessionBean implements IPaymentSessionBean {
			...
			do {
				...
				try {
					if (result != null) {
						...
						if (dto.getIsRefund() == 0 && paid.compareTo(BigDecimal.ZERO) > 0) {
	...
	public BigDecimal applyPayment(PaymentDTO payment, InvoiceDTO invoice, boolean success) throws SQLException {
		...
		if (invoice != null) {
			...
			if (success) {
				...
				if (invoice.getToProcess() == 0 || !UserBL.isUserBalanceEnoughToAge(invoice.getBaseUser(),null)) {
		...
		if (!totalPaid.equals(BigDecimal.ZERO)) {
			...
			PaymentLinkedToInvoiceEvent paymentLinkedToInvoiceEvent = new PaymentLinkedToInvoiceEvent(entityId,new PaymentDTOEx(payment),invoice,totalPaid);
			EventManager.process(paymentLinkedToInvoiceEvent);
			triggerEInvoicePlugin(entityId, paymentLinkedToInvoiceEvent);
	...
	private void triggerEInvoicePlugin(Integer entityId, PaymentLinkedToInvoiceEvent paymentLinkedToInvoiceEvent) {
		try {
			...
			while(null!= task) {
				task.generateEInvoiceOnSuccessfulPayment(paymentLinkedToInvoiceEvent);
