...
public class PaymentInvoiceMapDAS extends AbstractDAS<PaymentInvoiceMapDTO> {
    ...
    public List<PaymentInvoiceMapDTO> getLinkedPaymentsByInvoiceId(Integer invoiceId) {

        Criteria criteria = getSession().createCriteria(PaymentInvoiceMapDTO.class);
        criteria.createAlias("payment", "p");
        Criterion PAYMENT_SUCCESSFUL = Restrictions.eq("p.paymentResult.id", CommonConstants.PAYMENT_RESULT_SUCCESSFUL);
        Criterion PAYMENT_ENTERED = Restrictions.eq("p.paymentResult.id", CommonConstants.PAYMENT_RESULT_ENTERED);

        LogicalExpression successOrEntered = Restrictions.or(PAYMENT_ENTERED, PAYMENT_SUCCESSFUL);

        // Criteria or condition
        criteria.add(successOrEntered);
        criteria.add(Restrictions.eq("invoiceEntity.id", invoiceId));
