...
/***
 *
 * The plug in will migrate existing paypal instruments of the customers to respective Brain Tree Instruments
 * and generate required Vault Id for each customer for auto payments.
 *
 */

public class BrainTreePaymentMigrationExternalTask extends PaymentTaskWithTimeout implements IExternalCreditCardStorage{
...
//        return new PaymentInformationBL().useGatewayKey(payment.getInstrument());
    ...
    private PaymentAuthorizationDTO buildPaymentAuthorization(BrainTreeResult brainTreeResult) {
        ...
        String errorMsg = brainTreeResult.getErrorCode();
    ...
    public String storeCreditCard(ContactDTO contact, PaymentInformationDTO paymentInstrument) {
        ...
        payment.setPaymentMethod(new PaymentMethodDAS().find(Util.getPaymentMethod(new PaymentInformationBL().getCharMetaFieldByType(paymentInstrument, MetaFieldType.PAYMENT_CARD_NUMBER))));
        ...
        String customerKey = null != piBl.getCharMetaFieldByType(paymentInstrument, MetaFieldType.GATEWAY_KEY) ? new String(piBl.getCharMetaFieldByType(paymentInstrument, MetaFieldType.GATEWAY_KEY)) : null;
        ...
        try {
        	...
        	if(isCreditCardStored(paymentEx)) {
                logger.debug("Customer Key : {}",  piBl.getCharMetaFieldByType(payment.getCreditCard(), MetaFieldType.GATEWAY_KEY));
                return (null != piBl.getCharMetaFieldByType(paymentEx.getInstrument(), MetaFieldType.GATEWAY_KEY) ? new String(piBl.getCharMetaFieldByType(paymentEx.getInstrument(), MetaFieldType.GATEWAY_KEY)) : null);
            }
        	
            BrainTreeApi brainTreeAPI = getBTApi();

            customerKey = brainTreeAPI.createBTMigrationCustomer(convertCreditCard(paymentEx), convertPayer(paymentEx), getTimeoutSeconds()  * 1000, user.getId(), customerKey);
