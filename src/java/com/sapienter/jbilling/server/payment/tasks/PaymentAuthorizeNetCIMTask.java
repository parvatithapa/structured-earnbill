...
public class PaymentAuthorizeNetCIMTask extends PaymentTaskWithTimeout
    ...
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(PaymentAuthorizeNetCIMTask.class));
    ...
    private boolean doProcess(PaymentDTOEx info, String txType, String approvalCode) throws PluggableTaskException {
        ...
        if ( Constants.PAYMENT_METHOD_ACH.equals(info.getInstrument().getPaymentMethod().getId() ) ) {
            method = 2;
            if (piBl.isACH(info.getInstrument())) {
...
//        }
        ...
        char[] gatewayKey = piBl.getCharMetaFieldByType(info.getInstrument(), MetaFieldType.GATEWAY_KEY);
    ...
    private void deletePaymentProfile(PaymentInformationDTO instrument, AuthorizeNetCIMApi api, char[] gatewayKey) throws PluggableTaskException {
        try {
            CustomerProfileData customerProfile= CustomerProfileData.buildFromGatewayKey(gatewayKey);
...
class CustomerProfileData {
    ...
    public void setCreditCardNumber(char[] creditCardNumber) {
    ...
    public char[] getRoutingNumber() {
        return routingNumber;
    }

    public void setRoutingNumber(char[] routingNumber) {
    ...
    public void fillWith(ContactDTO contact, PaymentInformationDTO instrument, boolean customerEmail) {
        ...
        if (piBl.isACH(instrument)) {
            ...
            setRoutingNumber(piBl.getCharMetaFieldByType(instrument, MetaFieldType.BANK_ROUTING_NUMBER));
            setAccountNumber(piBl.getCharMetaFieldByType(instrument, MetaFieldType.BANK_ACCOUNT_NUMBER));
            setAccountName(piBl.getCharMetaFieldByType(instrument, MetaFieldType.TITLE));
        	...
        	char[] paymentCardNumber = piBl.getCharMetaFieldByType(instrument, MetaFieldType.PAYMENT_CARD_NUMBER);
    ...
    public static CustomerProfileData buildFromGatewayKey(char[] gatewayKey) {
        int delimiterPosition = Arrays.asList(gatewayKey).indexOf(GATEWAY_KEY_DELIMITER);
        String customerProfileId = new String(gatewayKey, 0, delimiterPosition);
        String paymentProfileId = new String(gatewayKey, delimiterPosition + GATEWAY_KEY_DELIMITER.length(), gatewayKey.length);
...
class AuthorizeNetCIMApi {
    private static final FormatLogger LOG = new FormatLogger(Logger.getLogger(AuthorizeNetCIMApi.class));
    ...
    private String buildCreateCustomerProfileRequest(CustomerProfileData customerProfileData, boolean customerEmail)
        ...
        if (customerProfileData.getPaymentType() == CustomerProfileData.CREDIT_CARD) {
            ...
            buildTag(XML, "cardNumber", new String(customerProfileData.getCreditCardNumber()));
            ...
            buildTag(XML, "routingNumber", new String(customerProfileData.getRoutingNumber()));
            buildTag(XML, "accountNumber", new String(customerProfileData.getAccountNumber()));
            buildTag(XML, "nameOnAccount", new String(customerProfileData.getAccountName()));
    ...
    private void parseGetCustomerProfileResponseNodeCreditCard(Node creditCardNode,
        ...
        for (int i = 0; i < creditCardNode.getChildNodes().getLength(); i++) {
            ...
            if ("cardNumber".equalsIgnoreCase(currentNode.getNodeName())) {
                ...
                customerProfileData.setCreditCardNumber(currentNode.getTextContent().toCharArray());
