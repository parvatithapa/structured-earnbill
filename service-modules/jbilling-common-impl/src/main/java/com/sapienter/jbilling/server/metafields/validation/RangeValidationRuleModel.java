...
public class RangeValidationRuleModel extends AbstractValidationRuleModel<Object> {
    ...
    public RangeValidationRuleModel() {
        ...
        setAttributeDefinitions(
                new MetaFieldAttributeDefinition(VALIDATION_MIN_RANGE_FIELD, MetaFieldAttributeDefinition.Type.DECIMAL, false),
                new MetaFieldAttributeDefinition(VALIDATION_MAX_RANGE_FIELD, MetaFieldAttributeDefinition.Type.DECIMAL, false)
        );
    }
    ...
    public ValidationReport doValidation(MetaContent source, Object object, ValidationRule validationRule, Integer languageId) {
        ...
        if (minValue != null) {
            minCondition = minValue.compareTo(new BigDecimal(object.toString())) <= 0;
        }
    ...
    public BigDecimal getValidationMinRangeField(ValidationRule validationRule) {
        return ParseHelper.getDecimal(validationRule.getRuleAttributes(), VALIDATION_MIN_RANGE_FIELD);
    }

    public BigDecimal getValidationMaxRangeField(ValidationRule validationRule) {
        return ParseHelper.getDecimal(validationRule.getRuleAttributes(), VALIDATION_MAX_RANGE_FIELD);
    }
